...
## 2026-08-07 状态更新：Focus Bridge 1.2.1

Android 已部署 `1.2.1 (14)`：锁屏等待最多 120 秒，解锁后显示 10 秒整数倒计时介入页；决定持久化重试，提交成功前不被后续 pending 覆盖；损坏说明回退为内置中文。真机已完成 ignored 与 accepted→5 分钟“不做手机控”→final event 两条闭环。
