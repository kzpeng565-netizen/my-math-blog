...
## 2026-08-08：每周循环任务的有效日期

==Obsidian 快照中的原始 `scheduled_date` 与 `recurrence` 保持事实源不变。Advisor 在构造 effective task view 时，只对精确匹配 `every week on <weekday>` 且日期已过去的任务做投影：以 Asia/Shanghai 的今天为基准，计算今天或下一次目标星期；对外的 `scheduled_date` 使用投影日期，并附带 `source_scheduled_date`、`recurrence_projected=true`。不识别的循环表达式保持原样，不猜测。==

<!-- ai_provenance: source=codex; date=2026-08-08; verification=task-sync-tests-and-live-three-task-check; retrieved_notes="非笔记内容/工作流程与系统运维/PI_SERVER_HANDOFF.md" -->

## 2026-08-09：完成实例与日计划快照

==任务完成实例保存在 Advisor task-sync state，键为 `task_id@YYYY-MM-DD`，不会因 Obsidian mutation ack 消失。`daily_plans` 对当天各任务的计划番茄数和完成番茄数只取观察到的最大值，避免当天删任务或下调番茄数后降低挑战门槛。==

==Garden 只读取 Advisor 输出的 `daily_scorecards`，在次日 04:10 后写入 SQLite `daily_achievements`。满足 `planned >= 7` 且 `completed >= planned` 时，用 `daily-full-tomato:<date>` 创建一次高级奖励；重复轮询不会重复发放。==

<!-- ai_provenance: source=codex; date=2026-08-09; verification=advisor-and-garden-unit-tests; retrieved_notes="非笔记内容/工作流程与系统运维/PI_SERVER_HANDOFF.md" -->

## 2026-08-31：目标模式证据与资料流

==Goal Agent 不读取 Garden 奖励账本或原始 ActivityWatch 作为学习完成证据。它读取用户显式反馈、task-sync 的稳定任务/完成事件、课程成绩与题源记录，以及资料清单中逐项授权的学习资料。番茄钟或行为记录至多作为辅助上下文，不能替代题目得分、交付物或任务完成。==

```mermaid
flowchart LR
  A["资料清单勾选"] --> B["Windows 导出器 v5"]
  C["Tasks 与直接完成"] --> B
  B -->|"分页/哈希/gzip + Syncthing"| D["Pi Goal FTS 与完成事件"]
  E["页面反馈"] --> F["EvidenceEvent"]
  D --> G["分轨道确定性进度"]
  F --> G
  G --> H["计划版本/审批/回退"]
```

==资料不足、少于三周可比数据、课程权重未知或题源不足时，输出保持 unknown/待核验。计划项接受日期后只排 task-sync mutation；直到 Obsidian 新快照确认，页面不得声称 Markdown 已写入。==

<!-- ai_provenance: source=codex; date=2026-08-31; verification=exporter-task-sync-and-goal-api-checked; retrieved_notes="../计划模式/01-整体架构与数据流.md" -->

## 2026-08-31：课程与 MathInk 可见层

```mermaid
flowchart LR
  A["目录/单文件授权"] --> B["Windows 导出器 v6"]
  C["Markdown + LaTeX"] --> B
  D["MathInk识别文字/图片引用"] --> B
  B -->|"移除笔迹/base64/布局坐标"| E["schema v2 gzip"]
  E --> F["Pi material_record + FTS"]
  G["用户授课小节+0–3掌握度"] --> H["course_progress_event/mastery"]
  F --> I["Goal Agent"]
  H --> I
```

==目录授权使用 Vault 相对路径和扩展名白名单；当前 45 份资料、0 错误。MathInk 压缩笔迹和图片二进制不进入同步，普通 Markdown、LaTeX、忠实识别段及标准相对图片引用保留。课程完成度不从资料数量或修改时间推断，只读取用户显式 course_progress。==

<!-- ai_provenance: source=codex; date=2026-08-31; verification=exporter-real-1.1-and-pi-material-index; retrieved_notes="../计划模式/01-整体架构与数据流.md" -->

## Goal v3 自动课堂与作业材料

从 2026-09-01 起，Windows exporter 对已授权课程目录递归导出安全可见文字，Pi 每 5 分钟依据稳定路径和 SHA-256 发现新课堂/作业候选。自动结果只进入待确认队列；确认前不改变掌握度、课程目录覆盖或 Obsidian Tasks。作业确认后才拆成多个学习任务并经既有 task-sync 写回。
