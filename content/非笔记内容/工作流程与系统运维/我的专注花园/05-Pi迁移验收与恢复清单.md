...
## 2026-08-31：目标模式部署验收与恢复点

- [x] ==Goal Agent 使用独立 SQLite、提示词、模型配置、聊天和审计历史；Next Action 未被替换。==
- [x] ==Garden 只固定代理 Goal API，服务仍为 `127.0.0.1:8838`，Advisor 仍为 `127.0.0.1:8767`。==
- [x] ==`:8460` 仍是 tailnet-only，Garden Funnel 未启用。==
- [x] ==计划 v1、4 条轨道、7 个里程碑、4 个 1590 分钟周和 12 个本周项可通过 Tailnet 读取。==
- [x] ==资料 0、证据 0 时四轨道与吞吐量保持未知。==
- [x] ==旧版本写请求返回 409 且不产生证据；SQLite `quick_check=ok`、权限 600。==
- [x] ==Advisor 199/199、Pi Garden 48/48、Goal 增量 12/12、Windows 导出器 11/11、Windows Garden 48/48。==
- [x] ==Tavily 同源密钥已安全配置并实测 HTTP 200，未进入仓库、Vault、日志或交接文档。==
- [ ] ==首次真实资料授权、accept-day、Obsidian ack、直接完成归因、自然周复盘、审批和回退仍待自然使用验收。==

==主要恢复点为 `/home/conrad/workspace/backups/goal-mode-20260830-221600/`。恢复代码或 unit 前先备份当前生产文件并逐文件比较；除非明确做数据库灾难恢复，不得覆盖 `data/goal_agent/goal-agent.sqlite3`。Windows 镜像只用于开发和代码恢复，不是 Pi 数据权威。==

<!-- ai_provenance: source=codex; date=2026-08-31; verification=pi-production-and-windows-mirror-final-check; retrieved_notes="../计划模式/02-当前完成与待补充.md" -->

## 2026-08-31：目标模式 v2 验收

- [x] ==schema v2 / plan v2，v1 历史与 48 个计划项保留。==
- [x] ==三门课程档案、110 个小节和 0–3 掌握度数据结构已建立。==
- [x] ==6 个课程任务等待授课进度，数学所/抽代等待真实资料；旧课程基线标题为 0。==
- [x] ==Windows 导出器 v6 已导出 45 份资料、0 错误，`1.1.md` 无笔迹/base64 泄漏。==
- [x] ==Goal Agent 配置为 GPT-5.6 Sol Responses/medium，独立 env 权限 600，无 DeepSeek fallback。==
- [x] ==Advisor 209/209、Pi/Windows Garden 49/49、导出器 13/13。==
- [x] ==Tailnet API 返回 schema v2、plan v2、3 课程档案和 45 资料。==
- [ ] ==第一次真实 course_progress、ready 任务写回和 2026-09-06 自然复盘待用户使用。==

==恢复点更新为 `/home/conrad/workspace/backups/goal-v2-20260831-134441/`。该目录含 SQLite backup API 生成且 quick_check=ok 的数据库副本。常规恢复只恢复代码/config/unit；计划策略回退使用 plan v1。==

<!-- ai_provenance: source=codex; date=2026-08-31; verification=pi-tests-tailnet-api-exporter-and-browser; retrieved_notes="../计划模式/05-v2课程进度与GPT-5.6迁移验收.md" -->


## 2026-08-31：课表导入与系统适配

==本次课表适配通过Advisor241项与Pi/Windows花园各50项回归，两个真实Tailnet接口均返回214次课程。备份为Advisor backups/20260831-course-schedule/与Windows D:\MyFocusGarden\backups\20260831-course-schedule\。只回退受影响代码，不覆盖部署后的课程反馈或花园数据库。==

详见 [[树莓派课程表导入与系统适配]]。

<!-- ai_provenance: source=codex; date=2026-08-31; verification=checked; retrieved_notes="我的专注花园/05-Pi迁移验收与恢复清单.md" -->
