...
==恢复点：`/home/conrad/services/focus-garden/backups/task-calendar-20260809-200247/`，包含部署前的 `static/app.js`、`static/index.html`、`static/style.css`。恢复时逐文件覆盖同名静态文件即可；不得覆盖 Pi SQLite、Advisor task-sync 状态、mutation queue 或 recent-context 数据。==

- [x] 今天分组包含两项当日循环任务，且总待办不重复计数。
- [x] 七天模式生成 7 日，逐项显示 Priority、番茄进度与循环标记。
- [x] 本月生成 31 日，格子显示任务数/预计番茄数，`highest` 有红点，点击日详情可见。
- [x] 日历尾部只显示 active 的近期动态。
- [x] 桌面无横向溢出；390px 移动端月历七列完整可见。
