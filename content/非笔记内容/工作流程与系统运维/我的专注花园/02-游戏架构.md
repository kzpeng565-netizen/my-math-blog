...
==focus_pauses 对每条 focus session 至多保留一条记录，包含暂停开始、确认分钟数与恢复截止时刻。暂停时完成结算被阻止，Windows target 的 Cold Turkey release 由既有 agent 执行；后台 reconciler 每 2 秒发现到期记录后恢复同一 blocks 与 targets，再将 ends_at 延后确认的暂停时长。==
