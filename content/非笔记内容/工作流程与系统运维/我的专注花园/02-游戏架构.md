...
`initial (revision 0) → clarify r1 → clarify r2 → accept r2`；任一阶段也可接受当前版本或换一个，但 accept 带 revision，服务端只接受 active 的同一版本。`r2` 后禁用继续澄清。模型只能改变建议字段，不能直接改变花园、任务 queue 或 Markdown。
