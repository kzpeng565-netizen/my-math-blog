...
==自 2026-08-05 起，奖励资格也分级：每 3 条尚未兑换的 `intervention_accepted` 记录派生 1 条 `intervention_basic` 初级资格；每 3 株已种下的初级植物派生 1 条 `advanced_unlock` 高级资格。原始介入记录只用于审计和计数，不能直接种植；服务端在写入 SQLite 前校验奖励资格与植物 `tier` 一致。升级时会按现有植物的 `tier` 回填历史，高级植物优先抵扣已累计资格，避免重复授予。==
