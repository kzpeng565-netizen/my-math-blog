...
==作息统计、批量教材转 Markdown、人文课抢课、与四条轨道无关的大规模额外自学和夜间固定 Agent 工作制明确排除。课程信息不齐时，允许在既定轨道与周容量内安排短小、可核验、可被真实课堂任务替换的启动任务。==
...
==概率论、泛函分析和微分几何的课内进度仍只由用户确认实际小节与 0–3 掌握度，文件时间、笔记数量和大纲学时不得自动解释为“已学完”。未填写进度的课内任务保持 awaiting_course_progress；但独立标记的启动任务可在授权资料 ready 后执行，并在真实课堂或作业出现时让位。==
...
## 2026-09-01：Goal Agent v3 决策

### D109. 实际课堂主题优先于预设课程目录【有效】

==课堂笔记分析只生成草稿。正式进度以用户确认的实际主题为主；课程目录和教材位置只作映射，只有确认映射才进入覆盖率。==

### D110. 作业按新文件名发现并拆成多个学习任务【有效】

==三门课程目录中含“作业”或含“习题但不含习题课”的新 Markdown 是作业候选。真实 DDL 和拆分必须确认；每个学习块不超过 180 分钟，并保留同一 Obsidian wikilink。==

### D111. 日期容量只由已承诺任务占用【有效】

==accepted/synced 任务才阻止新的日期确认。未确认推荐和 blocked/awaiting 项只用于推荐或等待，不得触发虚假的 180 分钟错误；真实冲突返回候选日期。==
