...
from .sleep_goal import build_sleep_goal_summary
...
class NextActionProxy:
    ...
    _PATHS = {
        "course_schedule": "/api/course-schedule",
    ...
    _GOAL_AGENT_PATHS = {
        ...
        "intake": "/api/goal-agent/intake",
        "intake_refresh": "/api/goal-agent/intake/refresh",
    }
    ...
    def goal_agent_scoped(
        ...
        *,
        ...
        allowed = (
            ...
            or re.fullmatch(r"/api/goal-agent/course-notes/candidate-[0-9a-f]{24}/(reanalyze|confirm)", path)
            or re.fullmatch(r"/api/goal-agent/material-candidates/candidate-[0-9a-f]{24}/(ignore|classify)", path)
            or re.fullmatch(r"/api/goal-agent/assignments/candidate-[0-9a-f]{24}/confirm", path)
            or re.fullmatch(r"/api/goal-agent/assignments/assignment-[0-9a-f]{24}/(replan|confirm-ready-blocks)", path)
        )
        if not allowed:
...
class GardenService:
    ...
    def sleep_goal(self, now: datetime | None = None) -> dict[str, Any]:
        """Project the user's semester sleep target from Advisor facts only."""
        return build_sleep_goal_summary(
            self.settings,
            self.advisor_data_root,
            now=now,
        )
...
class GardenHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/sleep-goal":
            return self._json(self.service.sleep_goal())
        ...
        if parsed.path == "/api/course-schedule":
            query = ("?" + parsed.query) if parsed.query else ""
            return self._next_action("course_schedule", "GET", query=query)
        ...
        if parsed.path == "/api/goal-agent/intake":
            return self._goal_agent("intake", "GET")
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if self.path == "/api/goal-agent/intake/refresh":
                return self._goal_agent("intake_refresh", "POST", body)
            if (
                ...
                or re.fullmatch(r"/api/goal-agent/course-notes/candidate-[0-9a-f]{24}/(reanalyze|confirm)", self.path)
                or re.fullmatch(r"/api/goal-agent/material-candidates/candidate-[0-9a-f]{24}/(ignore|classify)", self.path)
                or re.fullmatch(r"/api/goal-agent/assignments/candidate-[0-9a-f]{24}/confirm", self.path)
                or re.fullmatch(r"/api/goal-agent/assignments/assignment-[0-9a-f]{24}/(replan|confirm-ready-blocks)", self.path)
            ):
