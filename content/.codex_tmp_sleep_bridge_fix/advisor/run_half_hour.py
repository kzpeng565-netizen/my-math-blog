...
from course_schedule import window_context, context_lines
...
def _report_markdown(report: dict[str, Any], start: datetime, end: datetime) -> str:
    ...
    lines.extend(context_lines(report.get("course_schedule", {})))
    return "\n".join(lines)
...
def tagged_activity_minutes(tagged: dict[str, Any] | None, tag_name: str) -> float:
    """Sum one deterministic tag inside the report window, excluding context."""
    if not isinstance(tagged, dict):
        return 0.0
    seconds = 0.0
    for block in tagged.get("blocks", []):
        if not isinstance(block, dict) or block.get("scope") != "report":
            continue
        tags = block.get("tags", [])
        if any(
            isinstance(tag, dict) and tag.get("name") == tag_name
            for tag in tags if isinstance(tags, list)
        ):
            seconds += max(0.0, float(block.get("duration_seconds", 0) or 0))
    return round(seconds / 60, 2)


def apply_steam_activity_trigger(
    settings: dict[str, Any],
    intervention: dict[str, Any],
) -> dict[str, Any]:
    """Promote Steam usage over its strict threshold to a half-hour trigger.

    The timer firing is the monitoring event.  Steam usage is therefore an
    independent reason to intervene; it must not depend on a second behavior
    reason such as high stimulation or work/entertainment alternation.
    """
    rules = settings.get("computer_intervention", {})
    threshold: float | None = None
    if rules.get("enabled", True):
        for target in rules.get("targets", []):
            if not isinstance(target, dict) or target.get("trigger") != "steam_activity":
                continue
            threshold = float(target.get("minimum_activity_minutes", 5) or 5)
            break

    observations = intervention.setdefault("observations", {})
    used_minutes = float(observations.get("steam_activity_minutes", 0) or 0)
    triggered = threshold is not None and used_minutes > threshold
    observations["steam_activity_threshold_minutes"] = threshold
    observations["steam_activity_triggered"] = triggered
    if triggered:
        reasons = intervention.setdefault("trigger_reasons", [])
        if "steam_activity" not in reasons:
            reasons.append("steam_activity")
        intervention["would_intervene"] = True
    return intervention
...
def build_half_hour_reminder_check_message(
    ...
    lines = [
        ...
        f"Steam 使用：{observations.get('steam_activity_minutes', 0)} 分钟。",
        (
...
def run(arguments) -> dict[str, Any]:
    ...
    course_context = window_context(output_root, start, end)
    context["course_schedule"] = course_context
    atomic_write_json(context_archive_path, context)
    report_context = {**(context.get("ai_context") or {}), "course_schedule": course_context}
    ...
    intervention.setdefault("observations", {})["steam_activity_minutes"] = (
        tagged_activity_minutes(tagged, "steam_entertainment")
    )
    apply_steam_activity_trigger(settings, intervention)
    ...
    report["course_schedule"] = course_context
