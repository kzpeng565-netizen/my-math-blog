...
def run(arguments) -> dict[str, Any]:
    ...
    primary_evidence_seconds = (
        float(computer["activity"]["not_afk_minutes"])
        + float(phone["screen"]["on_minutes"])
    ) * 60
    tablet_on_seconds = float(tablet.get("screen", {}).get("on_minutes", 0)) * 60
    # Tablet is auxiliary: only used as fallback when primary devices show no activity
    evidence_seconds = primary_evidence_seconds
    if primary_evidence_seconds < int(settings["processing"]["minimum_evidence_seconds"]):
        if tablet_on_seconds > 0:
            evidence_seconds = tablet_on_seconds  # low-confidence fallback
