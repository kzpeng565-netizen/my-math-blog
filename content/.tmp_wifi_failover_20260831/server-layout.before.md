...
| Temporary fallback hotspot | NetworkManager connection `netplan-wlan0-XYH 0563` is retained with autoconnect priority 10. The touch panel has a `连接备用热点` recovery button. Do not remove this profile until UCAS operation is proven stable across reboots and portal-session expiry. |
