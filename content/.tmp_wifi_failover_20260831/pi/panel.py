...
def connect_backup_wifi() -> None:
    confirmed = messagebox.askyesno(
        "一键恢复热点连接",
        "将尝试连接电脑热点 XYH 0563。\n\n"
        "如果热点连接失败，系统会立即恢复 UCAS，不会把树莓派留在无网状态。\n\n"
        "确定继续吗？",
    )
    if not confirmed:
        return
    hotspot_button.configure(state="disabled", text="正在恢复热点…")

    def worker() -> None:
        success = False
        detail = "热点连接失败，已尝试恢复 UCAS。"
        try:
            result = subprocess.run(
                [
                    "/usr/bin/sudo",
                    "/usr/bin/python3",
                    str(WIFI_FAILOVER_SCRIPT),
                    "--config",
                    str(WIFI_FAILOVER_CONFIG),
                    "--force-fallback",
                ],
                check=False,
            ...
            success = (
                result.returncode == 0
                and f"{FALLBACK_PROFILE}:wlan0" in active
            )
            if success:
                detail = f"已连接电脑热点 XYH 0563。\n当前 IP：{local_ip()}"
            elif result.stderr.strip():
                detail = (
                    "热点连接失败，已尝试恢复 UCAS。\n"
                    f"错误：{result.stderr.strip()[-240:]}"
                )
        except (OSError, subprocess.TimeoutExpired) as error:
            detail = f"热点恢复超时，系统将保留或恢复可用连接。\n{error}"

        def finish() -> None:
            hotspot_button.configure(
                state="normal",
                text="一键恢复热点连接",
            )
            update_status()
            if success:
                messagebox.showinfo("热点连接已恢复", detail)
            else:
                messagebox.showerror("热点连接失败", detail)
