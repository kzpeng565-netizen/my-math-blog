...
| UCAS campus Wi-Fi | NetworkManager connection `UCAS` uses the open SSID `UCAS` and captive portal `https://portal.ucas.ac.cn/index_11.html`; autoconnect is enabled with priority 100. The observed DHCP IPv4 on 2026-08-30 was `10.207.252.167/16` (dynamic). Before portal login, IPv4 web traffic is intercepted, while native IPv6 remains usable and kept Tailscale reachable during testing. |
| Windows hotspot fallback | NetworkManager connection `netplan-wlan0-XYH 0563` is retained with autoconnect priority 10. Windows keeps Mobile Hotspot `XYH 0563` available through the current-user Startup watchdog under `D:\tools\pi-network-fallback`; no-client timeout is disabled. The Pi touch panel has an `一键恢复热点连接` button whose shared backend restores UCAS if hotspot activation fails. |
| Wi-Fi failover monitor | `wifi-failover.timer` runs every 30 seconds. `/home/conrad/workspace/activitywatch-advisor/scripts/wifi_failover.py` checks default route plus IPv4 204 and IPv6 HTTPS. Only four consecutive dual-stack failures trigger the Windows hotspot; Windows peer reachability is diagnostic only. State/logs are `/var/lib/wifi-failover/{state.json,events.jsonl}`. |
...
| Touch-panel application | `/home/conrad/touchpanel/panel.py`; `.xinitrc` loops and restarts it after exit. The `一键恢复热点连接` button invokes the shared failover script asynchronously and reports the resulting IP or safe UCAS restoration. |
| UCAS touch login | The panel's `UCAS 网络认证` button launches `/home/conrad/touchpanel/ucas_login.py`, switches to `UCAS`, and opens the official portal in Chromium with `matchbox-keyboard`. It uses a temporary Chromium profile under `/tmp/ucas-chromium-*`; returning to the panel terminates Chromium and the keyboard and removes that profile. SEP credentials are entered only in the official portal and are not stored in configuration or documentation. |
| UCAS browser packages | `chromium`, `chromium-sandbox`, `chromium-l10n`, `chromium-driver`, `fonts-liberation`, and `matchbox-keyboard`; installed 2026-08-30. |
| UCAS headless login | When no display is attached, use a one-shot headless Chromium session through ChromeDriver bound only to Pi loopback and an SSH local-forward. Use a unique `/tmp/ucas-headless-*` profile, terminate the temporary driver/browser after submission, and remove the profile. No persistent ChromeDriver service is enabled. Credentials must remain ephemeral and must never be copied into Pi files, commands, logs, or this reference. A headless portal login was verified on 2026-08-30 with IPv4 connectivity check 204 and external IPv4 HTTPS 200. |
...
| Goal Agent domain | Independent long-term goal controller in `/home/conrad/workspace/activitywatch-advisor/src/goal_agent.py` with Responses client `src/goal_model_client.py` and prompt `prompts/goal-agent.md`; it shares the Advisor web process on `127.0.0.1:8767` but does not share Next Action prompts, chats, database, or audit history |
| Goal Agent model | `gpt-5.6-sol` through the configured OpenAI-compatible Responses endpoint, reasoning effort `medium`; private key at `/home/conrad/.config/activitywatch-advisor/goal-agent.env` (mode 0600). Goal Agent does not inherit the global DeepSeek endpoint and does not fall back to DeepSeek |
| Goal Agent database and materials | SQLite schema v2 authority `/home/conrad/workspace/activitywatch-advisor/data/goal_agent/goal-agent.sqlite3` (mode 0600); includes course profiles, 110 course units, course progress/mastery, plan versions, evidence and chats. Authorized material schema v2 input under `/home/conrad/workspace/behavior-context-sync/goal_agent/`; Pi never writes Obsidian Markdown directly |
| Goal Agent weekly review | `goal-agent-review.timer` runs every Sunday at 20:30 Asia/Shanghai and invokes the independent review through `goal-agent-review.service`; public-source search credentials are private at `/home/conrad/.config/activitywatch-advisor/tavily.env` and must never be printed or copied into documentation |
