...
def _bridge_heartbeat_metadata(body: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("accessibility_enabled", "accessibility_connected", "fallback_active"):
        if isinstance(body.get(key), bool):
            metadata[key] = body[key]
    for key in ("service_uptime_seconds", "restart_count", "connection_count"):
        value = body.get(key)
...
class GardenService:
    ...
    def start_focus(self, profile_id: str, duration: int, targets: list[str] | None = None,
        ...
        selected_targets = set(["windows", "phone"] if targets is None else targets)
        ...
        session = self.db.create_focus(profile_id, duration, now.isoformat(timespec="seconds"),
                                       ...
                                       targets=sorted(selected_targets), blocks=self.profile_map[profile_id]["blocks"])
        try:
            if not selected_targets:
                ...
                status, payload, _ = self.next_action.intervention(
                    ...
                    body={"duration": duration, "targets": sorted(selected_targets),
                          "blocks": self.profile_map[profile_id]["blocks"]},
                )
    ...
    def release_focus_lease(self, session: dict[str, Any]) -> None:
        """Queue a Windows-only release through the existing intervention agent."""
        if "windows" not in set(session.get("targets", [])):
            return
