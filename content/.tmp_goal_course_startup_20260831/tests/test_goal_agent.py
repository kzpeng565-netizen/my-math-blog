...
class GoalAgentTest(unittest.TestCase):
    ...
    def test_seed_has_exact_capacity_and_unknown_evidence(self) -> None:
        ...
        probability = next(
            profile for profile in state["course_profiles"]
        ...
        current = {item["id"]: item for item in state["current_week"]["items"]}
        self.assertIn("GTM259 §2.1", current["w1-t-1"]["title"])
        self.assertEqual(current["w1-t-1"]["recommended_date"], "2026-08-31")
        self.assertIn("概率论启动", current["w1-c-p1"]["title"])
        self.assertEqual(current["w1-c-p1"]["recommended_date"], "2026-09-02")
        self.assertEqual(current["w1-c-p1"]["input_state"], "awaiting_material")
        self.assertNotIn(
            "课程基线检索练习",
    ...
    def test_three_weeks_of_evidence_adjust_next_week_inside_confirmed_range(self) -> None:
        ...
        result = self.agent.feedback({
            ...
            "base_plan_version": self.agent.plan()["plan_version"],
    ...
    def test_probability_bootstrap_becomes_ready_then_yields_to_course_progress(self) -> None:
        self.current = datetime(
            2026, 8, 31, 20, 0, tzinfo=ZoneInfo("Asia/Shanghai")
        )
        with self.agent._connect() as connection:
            connection.execute(
                "UPDATE plan_item SET material_status='ready' WHERE course_id='probability'"
            )
            connection.execute(
                "UPDATE plan_item SET input_state='ready' "
                "WHERE id IN ('w1-c-p1','w1-c-p2')"
            )
        items = {
            item["id"]: item for item in self.agent.state()["current_week"]["items"]
        }
        self.assertEqual(items["w1-c-p1"]["input_state"], "ready")
        self.assertIn("概率论启动", items["w1-c-p1"]["title"])
        self.agent.feedback(
            {
                **self.request("probabilitycourse"),
                "track_id": "track-courses",
                ...
                "details": {
                    "course": "概率论",
                    "taught_units": [
                        {"unit_id": "probability-01-01", "mastery": 1}
                    ],
                },
            }
        )
        ...
        self.assertNotIn("概率论启动", rewritten["w1-c-p1"]["title"])
        self.assertIn("组合分析", rewritten["w1-c-p1"]["title"])
