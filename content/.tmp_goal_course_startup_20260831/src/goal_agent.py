...
class GoalAgent:
    ...
    def _initialize(self) -> None:
        with self._lock, self._connect() as connection:
            ...
            self._migrate_probability_bootstrap_plan(connection)
            connection.execute(
        ...
        for profile in COURSE_PROFILES:
            connection.execute(
                ...
                (
                    ...
                    "user_confirmed",
                    (
                        "用户于 2026-08-31 更正课程教材：Le Gall 为主教材，"
                        "Durrett 为参考书；考试日期仍未知。"
                        if profile["id"] == "probability"
                        else "用户于 2026-08-31 提供并确认的课程页面内容；考试日期仍未知。"
                    ),
                    now,
                ),
            )
    ...
    def _migrate_probability_bootstrap_plan(
        self,
        connection: sqlite3.Connection,
    ) -> None:
        """Make the first week executable before all course details arrive.

        Bootstrap items prove only that a study block was completed. They do
        not count as taught course units and are replaced by the existing
        course-progress rewrite once the user submits real class scope.
        """
        meta_key = "probability_bootstrap_plan_20260831"
        ...
        updates = {
            "w1-t-1": {
                "title": "遍历论：收尾 GTM259 §2.1 习题并回看 Ch.1 §1.1–1.2",
                "description": (
                    "先完成已选 §2.1 保测系统与同构习题，再用一段话复述遍历论研究对象"
                    "与两个动机例子；材料来自已授权 GTM259 分页资料和现有笔记。"
                ),
                "recommended_date": "2026-08-31",
                "bootstrap": True,
            },
            "w1-t-2": {
                "title": "遍历论：学习 GTM259 §2.2 Recurrence",
                "description": (
                    "复述 Poincaré recurrence theorem，并重建坏点集原像两两不交、"
                    "有限测度迫使坏点集测度为 0 的证明。"
                ),
            ...
            "w1-c-p1": {
                "title": "概率论启动：复习测度、概率测度与可测映射",
                "description": (
                    "在真实授课小节仍待补充时，复习 σ-代数、测度/概率测度、"
                    "可测映射与随机变量；这不计作老师已讲内容。"
                ),
                "recommended_date": "2026-09-02",
                "bootstrap": True,
            },
            "w1-t-3": {
                "title": "遍历论：完成 GTM259 §2.2 习题与证明复述",
                "description": (
                    "在完成 §2.2 阅读后尝试本节习题，记录证明卡点和下一次待讨论问题；"
                    "完成反馈前不提前启动 §2.3。"
                ),
            ...
            "w1-c-p2": {
                "title": "概率论启动：复习积分与收敛定理",
                "description": (
                    "复习简单函数逼近、Fatou 引理、单调收敛和控制收敛；"
                    "课程上课后由真实小节与作业自动接管。"
                ),
                "recommended_date": "2026-09-04",
                "deep_minutes": 87,
                "bootstrap": True,
            },
            # Move waiting items rather than exceeding the weekday 180-minute cap.
            "w1-e-2": {"recommended_date": "2026-09-06"},
            "w1-c-d1": {"recommended_date": "2026-09-05"},
            "w1-c-f1": {"recommended_date": "2026-09-06"},
        }
        changes: list[dict[str, Any]] = []
        for item_id, update in updates.items():
            row = connection.execute(
                "SELECT p.*,pt.plan_item_id AS mapped_item FROM plan_item p "
                "LEFT JOIN plan_item_task pt ON pt.plan_item_id=p.id WHERE p.id=?",
                (item_id,),
            ).fetchone()
            if not row or row["accepted_date"] or row["mapped_item"]:
                continue
            before = {
                "title": row["title"],
                "description": row["description"],
            ...
            title = update.get("title", row["title"])
            description = update.get("description", row["description"])
            deep_minutes = int(update.get("deep_minutes", row["deep_minutes"]))
            input_state = row["input_state"]
            if update.get("bootstrap"):
                input_state = (
                    "ready"
            ...
            connection.execute(
                "UPDATE plan_item SET title=?,description=?,recommended_date=?,"
                "deep_minutes=?,input_state=?,updated_at=? WHERE id=?",
                (
                    title,
                    description,
                    update["recommended_date"],
                    deep_minutes,
                    input_state,
                    now,
                    item_id,
            ...
            after = {
                "title": title,
                "description": description,
                "recommended_date": update["recommended_date"],
                ...
                changes.append(
                    {
                        ...
                        "field": "bootstrap_study",
                        "before": before,
                        "after": after,
                        "reason": (
                            "课程信息可边补充边学习；启动任务不冒充课内进度，"
                            "真实授课小节或作业出现后自动让位。"
                        ),
                    }
                )
        ...
        if changes and self._current_version(connection):
            self._create_version(
                connection,
                "更正概率论教材并启动 GTM259/概率论先行学习",
                "user_course_correction",
                changes,
                "codex",
            )
    ...
    def ingest_material_exports(self) -> dict[str, Any]:
        ...
        with self._lock, self._connect() as connection:
            ...
            for course_name, course_id in COURSE_IDS.items():
                ...
                if course_id == "probability" and not has_progress:
                    connection.execute(
                        "UPDATE plan_item SET input_state=? "
                        "WHERE id IN ('w1-c-p1','w1-c-p2') "
                        "AND archived=0 AND accepted_date IS NULL",
                        ("ready" if ready else "awaiting_material",),
                    )
