...
class DailyLifeNotifierTests(unittest.TestCase):
    ...
    def test_pending_sleep_boundary_waits_without_push_before_11(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            markdown = root / "statistics" / "daily_life" / "2026-07-28.md"
            report_json = root / "statistics" / "daily_life" / "2026-07-28.json"
            markdown.parent.mkdir(parents=True)
            markdown.write_text("pending", encoding="utf-8")
            report_json.write_text(
                json.dumps(
                    {
            ...
            with patch(
                ...
                receipt = notify_daily_life(
                    ...
                    now=datetime.fromisoformat("2026-07-29T10:00:00+08:00"),
                )

            self.assertEqual(receipt["status"], "pending_sleep_boundary")
            self.assertEqual(receipt["next_retry_hour"], 11)
            self.assertEqual(run_mock.call_count, 1)
            self.assertTrue(
                (root / "state" / "daily_life_morning" / "2026-07-28.json").exists()
            )
