...
class ContextTests(unittest.TestCase):
    ...
    def test_low_efficiency_alone_does_not_trigger_intervention(self):
        settings = {
            "behavior_advisor": {
                "enabled": True,
                "shadow_mode": True,
    ...
    def test_pomodoro_is_medium_reliability_positive_evidence(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            live, cache = root / "context_snapshot.json", root / "cache.json"
            live.write_text(json.dumps(snapshot()), encoding="utf-8")
            ...
            quality = result["ai_context"]["pomodoro"]["data_quality"]
            self.assertEqual(quality["reliability"], "medium")
            self.assertEqual(quality["pomodoro_minutes"], 40)
            self.assertIn("40 minutes", quality["unit_rule"])
            self.assertIn("positive_evidence_only", quality["decision_role"])

    def test_work_entertainment_alternation_triggers_intervention(self):
        settings = {
            "behavior_advisor": {
                "enabled": True,
                "shadow_mode": True,
                "work_entertainment_transition_threshold": 2,
            }
        }
        semantic = {
            "segments": [
                {"activity": "work", "duration_seconds": 600},
                {
                    "activity": "entertainment",
                    "duration_seconds": 120,
                    "task": "浏览知乎",
                },
                {"activity": "work", "duration_seconds": 1080},
            ]
        }
        mixing = {
            "work_entertainment_transition_count": 2,
            "qualified_work_entertainment_transition_count": 2,
            "entertainment_deviation_count": 1,
        }
        cross = {
            "time_accounting_observed": {
                "computer_not_afk_minutes": 30,
                "phone_screen_on_minutes": 0,
                "confirmed_rest_minutes": 0,
            }
        }
        result = build_shadow_candidate(
            ...
            mixing,
            cross,
            {"available": False, "context_source": "unavailable"},
        )
        self.assertTrue(result["would_intervene"])
        self.assertIn("work_entertainment_alternation", result["trigger_reasons"])
    def test_confirmed_rest_exemption_requires_more_than_twelve_minutes(self):
        settings = {
            "behavior_advisor": {
                "enabled": True,
                "shadow_mode": True,
                "confirmed_rest_exemption_minutes": 12,
            }
        }
        context = {"available": False, "context_source": "unavailable"}
        semantic = {
            "segments": [
                {"activity": "entertainment", "duration_seconds": 720, "task": "浏览知乎"}
            ]
        }
        short_rest = {
            "time_accounting_observed": {
                "computer_not_afk_minutes": 25,
                "phone_screen_on_minutes": 0,
                "confirmed_rest_minutes": 12,
            }
        }
        result = build_shadow_candidate(
            settings, datetime.now().astimezone(), semantic, {}, short_rest, context
        )
        self.assertTrue(result["would_intervene"])
        self.assertFalse(result["observations"]["has_explicit_rest"])

        explicit_rest = {
            "time_accounting_observed": {
                "computer_not_afk_minutes": 25,
                "phone_screen_on_minutes": 0,
                "confirmed_rest_minutes": 12.01,
            }
        }
        result = build_shadow_candidate(
            settings, datetime.now().astimezone(), semantic, {}, explicit_rest, context
        )
        self.assertFalse(result["would_intervene"])
        self.assertTrue(result["observations"]["has_explicit_rest"])
