...
<!-- ai_provenance: source=codex; date=2026-08-28; verification=device-and-pi-tested; retrieved_notes="Pi activitywatch-advisor production state and phone usage receiver" -->
...
Advisor `src/task_sync.py` 新增 `completions`、`daily_plans`，有效状态新增 `completed_today`、`daily_scorecards`。Garden SQLite 新增 `daily_achievements`，次日 04:10 后幂等结算；达标奖励 ID 为 `daily-full-tomato:YYYY-MM-DD`，可直接种高级植物。

前端已支持循环“完成本次”、今天已完成任务灰显无编辑、六日/月历大勾和直接高级奖励种植。备份：Advisor `backups/20260809-232309-task-completion/`；Garden `backups/20260809-232309-daily-achievement/`。Advisor 10/10、Garden 34/34；两服务 active。Garden 文件已同步到 `D:\MyFocusGarden`。

## 2026-08-10：手动专注手机锁机剩余时长梯次

==手动专注 execute 现在携带绝对 `focus_deadline_at`、`phone.allowed_minutes` 与 `nearest_remaining_midpoint_tie_longer`。Android 首次实际尝试前按剩余时间选最近档，精确中点取较长；默认 `5,20,30,40,45,60`。例如 60 分钟请求延迟 7.5 分钟仍锁 60，延迟 7.6 分钟锁 45。配置入口是 Focus Garden 权威树 `config/settings.json` 的 `focus.allowed_minutes`，bootstrap 与网页按钮共用该配置。Getaway 当前只支持已编译的六个快捷档，新增任意新分钟值仍需同时扩展 Android 快捷网格映射。==

==结束、暂停、取消会给 phone 与 windows 分别创建稳定 release request；手机 release 可取消仍在等待或重试的原 execute，Advisor 同时完成 release ID 与被取代的 execute ID，阻止过期请求再次 pending。生产备份位于 Focus Garden 与 Advisor 各自 `.deploy-backups/20260810-0040-duration-ladder/`。Advisor 13/13、Garden 20/20、Android 策略检查与离线 Gradle 构建通过，两服务 active、无 warning。==

==Android `1.3.4 (19)` 已覆盖安装，最终 APK SHA-256 为 `AC31020786224B585E37E7F5A6103DCE7318B622AFF52CBBD3827A41E10B5AE8`。Pi 心跳确认 `app_version=1.3.4`、状态 online、`no_pending`、无障碍与通知监听均已连接；梯次与取消策略等待自然场景验收。==

<!-- ai_provenance: source=codex; date=2026-08-10; verification=pi-deployed-tested-android-install-pending; retrieved_notes="PROJECT_STATE.md,DECISIONS.md,NEXT_STEPS.md,我的专注花园/专注花园桥接手机APP.md" -->
...
Garden `static/app.js` / `style.css` 已在当前待办和日历任务卡显示“拖延 · N天”。Next Action 的候选排序、硬规则、校验与 fallback 全部优先拖延任务。Advisor 35/35、Garden 36/36、Windows `node --check`、两服务 restart、loopback 与 Tailnet `:8450/:8460` HTTP 200 均通过。

部署前备份：`/home/conrad/workspace/backups/procrastination-label-20260812-003216/`，含受影响文件与部署前 `task_sync/state.json`。回滚不得覆盖 Garden SQLite。

<!-- ai_provenance: source=codex; date=2026-08-12; verification=pi-tests-services-and-tailnet -->


## 2026-08-14：Steam 半小时监控、初版夜间 lease 与 Focus Garden 强制附加块

Advisor `config/tag_rules.json` 新增 Steam 硬娱乐规则；`src/run_half_hour.py` 写入 `observations.steam_activity_minutes`，严格大于 5 分钟时追加独立原因 `steam_activity` 并直接令 `would_intervene=true`，不要求其他行为理由。`src/computer_intervention.py` 随后加入带 `pre_lock_countdown_seconds=60` 的 `steam游戏`。

Windows `D:\tools\computer-intervention-agent\{agent.py,intervention_ui.pyw,config.json}` 当日初版加入 Steam allowlist、forced 倒计时和本地 `23:30—次日12:00` 计划 lease；该夜间 lease 已于 2026-08-15 被下节硬锁流程替代。Focus Garden `config/settings.json` 设置 `focus.always_windows_blocks=["steam游戏"]`，每个 session 都附加 Steam，服务仍经固定 loopback Advisor API 派发。

验证：Windows Agent 14/14、Advisor 19/19、本次 Garden 新增测试、Steam 样例确定性标签与 5.02 分钟目标通过；两服务 active，loopback 8767/8838 与 Tailnet 8450/8460 均为 HTTP 200。Garden 全套 21 项中唯一旧失败读取了生产 3 条 pending mutation 却断言 0，与本功能无关。Windows 回滚目录为 `D:\tools\computer-intervention-agent\backups\20260814-141833-steam-monitor\`；本次 Pi 部署前专用备份因命令引号错误未成功生成，不得写成可用恢复点。

2026-08-14 复核发现旧实现把 Steam `>5` 错当成总体 `would_intervene` 的附加门槛：14:38 首轮触发后，15:08 对应窗口虽有 16.93 分钟 Steam，却被错误跳过。现已改为 Steam 超阈值独立触发；历史回放中 14:30、15:00、15:30 三轮均成为 `steam_activity` 候选。Pi 共享拒绝状态同时补齐 90 分钟、有意义活动和确认休息三类重置，保证 15:08 属于连续窗口，而跨过 90 分钟的 18:08 从首次提醒重新计数。相关 Advisor 测试 23/23 通过；18:08 自然运行进一步确认 Steam 10.28 分钟、`steam_activity_triggered=true`、ntfy accepted、Windows ack。部署前备份为 `/home/conrad/workspace/backups/20260814-steam-continuous-trigger-pre/`；不补发历史锁机。

<!-- ai_provenance: source=codex; date=2026-08-14; verification=windows-unit-tests-pi-targeted-tests-services-tailnet-and-deterministic-sample -->

## 2026-08-15：Steam 夜间硬锁、主要任务与白天解锁门槛

Windows Agent 的夜间流程改为 `steam_night_prompt`：23:30 显示 60 秒收尾框，主动关闭 `C:\steam\steamapps\common\Magical Girl Celesphonia\Game.exe` 可调用 Garden 奖励接口获得一次普通植物机会；用户也可按 15 分钟档延时，最晚 01:00。无操作或延时到点后先完成 60 秒存档倒计时，再关闭精确路径对应进程并调用 Cold Turkey `-start "steam游戏" -lock <minutes>` 硬锁至 12:00。UI 按 Tk 控件请求高度动态定窗，并在倒计时下方增加留白，避免“秒”下沿被裁切；遗留 schedule 状态会迁移且不重复弹窗。

Advisor `task_sync` 提供 `primary_tasks`；Garden 对外生成权威 `steam_unlock_gate`：只允许今天/明天各一个主要任务；当天整项完成的任务按各自 `tomatoes_total` 一次性授予番茄，累计达到 5 个且主要任务完成才令 `eligible=true`。未完成任务的 `tomatoes_completed`（如 `2/4`）不参与解锁计数，指标封顶显示 `5/5`。Garden 新增主要任务代理与界面、Agent 专用 gate 端点及夜间主动关闭奖励端点；奖励按 event ID 幂等。12:00 后 Agent 每分钟读取 gate；不满足或 Pi 不可达时续上 5 分钟硬锁，满足后停止续锁，最迟约 5 分钟自然释放。Next Action 只把主要任务当软性排序依据。

生产文件已部署到 `/home/conrad/workspace/activitywatch-advisor` 与 `/home/conrad/services/focus-garden`，两服务 active；Garden 6 个变更文件已同步至 `D:\MyFocusGarden`。验证为 Windows Agent 19/19、Advisor 37/37、Garden database 10/10 与两项关键 service 测试通过；Garden 全套仍只有既有的生产 pending mutation 数量断言失败。浏览器验收确认“设为主要”只出现在今天/明天，位于原四按钮上一行右侧。备份：Windows `D:\tools\computer-intervention-agent\backups\20260815-112333-steam-hard-gate\`；Pi Garden `/home/conrad/services/focus-garden/backups/20260815-1134-steam-hard-gate/`；Pi Advisor `/home/conrad/workspace/backups/20260815-1134-steam-hard-gate-activitywatch-advisor/`；Windows Garden 镜像 `D:\MyFocusGarden\backups\20260815-1150-steam-gate-main-task\`。

<!-- ai_provenance: source=codex; date=2026-08-15; verification=windows-pi-tests-live-services-api-browser-and-mirror-hashes -->

## 2026-08-15：系统状态末尾新增 Steam 解锁指标

Focus Garden `GardenService.system_status()` 现返回任务正文脱敏的 `steam_unlock`：`available/date/completed_tomatoes/required_completed_tomatoes/tomato_requirement_met/primary_task_set/primary_task_completed/eligible/blocking_reasons`。它通过既有 loopback task-sync gate 读取，健康页专用超时为 3 秒；失败时统一返回“指标暂不可用，继续保持 Steam 锁定”。

`static/lease-status.js` 在电脑 lease 卡之后追加宽卡，因此“Steam 解锁指标”始终是健康面板最后一张卡；`index.html` 使用 `lease-status.js?v=20260815.2`。复核发现原来的 `2/6` 来自未完成任务的 `2/4` 中途进度，按规则不应计入。最终规则为任务整项完成时一次性按 `tomatoes_total` 授予；当前 5 番茄任务完成后指标直接为 `5/5`，主要任务也已完成，gate 为 eligible。Garden 全套 47/47 测试通过；服务 active、health 200，Tailnet Agent API 与系统状态均返回 `5/5`，并带 `tomato_count_source=completed_tasks_planned_tomatoes`。备份为 `/home/conrad/services/focus-garden/backups/20260815-2103-steam-gate-completed-tasks/`。

生产树在部署过程中另行合入 control-system 模块；本次以 13:17 最新生产文件为基线重新合并，没有覆盖该模块。最终 Pi 备份为 `/home/conrad/services/focus-garden/backups/20260815-1324-steam-unlock-after-control-system/`，Windows 镜像备份为 `D:\MyFocusGarden\backups\20260815-1327-control-system-plus-steam-status\`。

<!-- ai_provenance: source=codex; date=2026-08-15; verification=pi-tests-service-health-tailnet-api-browser-and-mirror-hashes -->

## 2026-08-15：控制状态手动同步

生产新增 `POST /api/control/sync` 和“同步状态”按钮。接口强制刷新当天 D 并重算八项聚合到 `data/control-live.json`；活动中的周快照继续提供状态、证据、唯一调整和冻结截止时间。新周快照成功写入时删除旧 live 快照。
