...
==Steam 候选的 `observations` 包含 `steam_activity_minutes`、`steam_activity_threshold_minutes` 与 `steam_activity_triggered`。时长只累计本窗口 `scope=report` 且带 `steam_entertainment` 的 tagged facts；严格超过 5 分钟时追加 `trigger_reasons=[..., "steam_activity"]` 并独立令 `would_intervene=true`。==
...
<!-- ai_provenance: source=codex; date=2026-08-28; verification=pi-code-and-live-merge-test; retrieved_notes="Pi activitywatch-advisor production state and phone usage receiver" -->
...
## 2026-08-09：任务完成与每日挑战接口

- Advisor `GET /api/task-sync/state` 有效视图新增 `completed_today`、`daily_scorecards`；完成键为 `task_id@YYYY-MM-DD`。
- mutation 接口不变：普通任务排 `complete`；精确每周循环任务先记本次实例，再排 `update` 推进模板。
- Garden `/api/bootstrap` 新增 `daily_achievements`；记录日期、计划/完成番茄数、任务数、达标状态及 reward。
- 每日达标 reward 为 `tier=advanced`，直接种植高级植物，不经过普通奖励兑换。

## 2026-08-15：系统状态中的 Steam 解锁摘要

- Focus Garden `GET /api/system-status` 新增 `steam_unlock`。
- 字段：`available`、`date`、`completed_tomatoes`、`required_completed_tomatoes`、`tomato_requirement_met`、`primary_task_set`、`primary_task_completed`、`eligible`、`blocking_reasons[]`。
- 摘要不返回 `primary_task_id`、任务标题或任务正文；上游不可用时 `available=false`、`eligible=false`，并说明继续保持锁定。
- Agent 专用 `/api/steam-gate/status` 认证和完整 gate 结构不变。

## 2026-08-15：Focus Garden 控制状态同步

- `POST /api/control/sync` 只接受 JSON `{}`，强制更新当天 D 并重新聚合 M/D/W/L/A/F/U/R。
