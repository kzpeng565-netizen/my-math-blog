...
from task_sync import acknowledge_mutations, effective_state, enqueue_mutation, set_primary_task
    ...
    def do_POST(self) -> None:
        ...
        try:
            if parsed.path == "/api/next-action":
                ...
                return
            clarify_match = re.fullmatch(r"/api/next-action/([A-Za-z0-9_-]+)/clarify", parsed.path)
            if clarify_match:
                body = self._body()
                result = clarify_next_action(
                    state.settings,
                    state.output_root,
                    clarify_match.group(1),
                    str(body.get("message", "")),
                    body.get("expected_action_revision"),
                    env_file=state.env_file,
                )
                self._json(HTTPStatus.OK, result)
                return
            if parsed.path == "/api/task-sync/mutations":
                ...
                return
            if parsed.path == "/api/task-sync/primary":
                self._json(
                    HTTPStatus.OK,
                    set_primary_task(
                        _context_path(state),
            ...
            if parsed.path == "/api/interventions/manual-focus":
                ...
                request = build_manual_focus_request(
                    ...
                    focus_deadline_at=(str(body.get("focus_deadline_at", "")).strip() or None),
                    allowed_phone_minutes=[int(item) for item in body.get("allowed_phone_minutes", [])] or None,
                )
            ...
            if parsed.path == "/api/interventions/manual-focus/release":
                ...
                targets = body.get("targets", ["windows"])
                if not isinstance(targets, list):
                    raise ValueError("targets must be a list")
                request = build_manual_focus_release_request(
                    ...
                    requested_targets=[str(item) for item in targets],
                )
                path = save_computer_intervention_request(state.output_root, request)
            ...
            if match:
                ...
                self._json(
                    ...
                    save_response(
                        ...
                        expected_action_revision=body.get("expected_action_revision"),
                    ),
                )
                return
