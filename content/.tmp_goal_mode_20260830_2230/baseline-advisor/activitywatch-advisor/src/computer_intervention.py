...
LEGACY_RELEASE_QUARANTINE_AFTER = timedelta(minutes=10)
...
def _target_is_triggered(
    ...
    if trigger == "steam_activity":
        observations = intervention.get("observations", {})
        used_minutes = float(observations.get("steam_activity_minutes", 0) or 0)
        threshold = float(target.get("minimum_activity_minutes", 5) or 5)
        return used_minutes > threshold
    return False
...
def build_computer_intervention_request(
    ...
    for target in rules.get("targets", []):
        if not isinstance(target, dict) or not _target_is_triggered(
            target, semantic, intervention
        ):
            continue
        ...
        targets.append(
            {
                ...
                "pre_lock_countdown_seconds": int(
                    target.get("pre_lock_countdown_seconds", 0) or 0
                ),
            }
        )
    if not targets:
...
def build_manual_focus_request(
    ...
    *,
    ...
    focus_deadline_at: str | None = None,
    allowed_phone_minutes: list[int] | None = None,
    ...
    deadline = datetime.fromisoformat(focus_deadline_at) if focus_deadline_at else moment + timedelta(minutes=duration)
    if deadline.tzinfo is None:
        raise ValueError("focus deadline must include a timezone")
    deadline = deadline.astimezone(moment.tzinfo)
    if deadline <= moment:
        raise ValueError("focus deadline must be in the future")
    phone_minutes = sorted({int(value) for value in (allowed_phone_minutes or [5, 20, 30, 40, 45, 60])})
    if any(value < 1 or value > 180 for value in phone_minutes) or duration not in phone_minutes:
        raise ValueError("phone duration must be present in the allowed minute ladder")
    ...
    return {
        ...
        "period": {
            ...
            "end": deadline.isoformat(timespec="seconds"),
        },
        "focus_deadline_at": deadline.isoformat(timespec="seconds"),
        "mode": "execute",
        "source": "manual_focus",
        "lease_id": request_id,
        "expires_after_seconds": max(180, int((deadline - moment).total_seconds()) + 60),
        "targets": [{**target, "lease_id": request_id} for target in windows_targets],
        "phone": {
            "enabled": "phone" in targets,
            "minutes": duration,
            "allowed_minutes": phone_minutes,
            "duration_strategy": "nearest_remaining_midpoint_tie_longer",
        },
...
def build_manual_focus_release_request(
    ...
    session_id: str | None = None, requested_targets: list[str] | None = None,
    now: datetime | None = None
) -> dict[str, Any]:
    ...
    targets = set(requested_targets or ["windows"])
    if not targets or not targets <= {"windows", "phone"}:
        raise ValueError("release targets must contain windows and/or phone")
    blocks = [str(block) for block in requested_blocks]
    if "windows" in targets and (not blocks or any(block not in allowed_by_name for block in blocks)):
        raise ValueError("release blocks must be in the configured allowlist")
    moment = now or datetime.now().astimezone()
    owner = str(lease_id or "").strip()
    session = str(session_id or "").strip()
    if not owner:
        # A release without the exact start lease cannot prove it belongs to
        # the current Cold Turkey session.  Refuse to create an unsafe generic
        # `-stop` request rather than leaving a replayable command behind.
        raise ValueError("release requires the active lease_id")
    stable_owner = owner or session
    if stable_owner:
        safe_owner = "".join(ch for ch in stable_owner if ch.isalnum() or ch in "-_")[:100]
        target_suffix = "-".join(sorted(targets))
        request_id = f"manual-focus-release-{safe_owner}-{target_suffix}"
    else:
    ...
    return {
        ...
        "targets": [
            {"name": name, "cold_turkey_block": str(allowed_by_name[name].get("cold_turkey_block", name)),
             ...
             "lease_id": owner or None}
            for name in blocks if "windows" in targets
        ],
        "phone": {
            "enabled": "phone" in targets,
            "minutes": 0,
            "cancel_request_id": owner if "phone" in targets else None,
        },
...
def _release_lease_id(request: dict[str, Any]) -> str:
    owner = str(request.get("lease_id") or "").strip()
    if owner:
        return owner
    target_ids = {
        str(target.get("lease_id") or "").strip()
        for target in request.get("targets", [])
        if isinstance(target, dict) and str(target.get("lease_id") or "").strip()
    }
    return target_ids.pop() if len(target_ids) == 1 else ""


def _is_legacy_unleased_release(request: dict[str, Any]) -> bool:
    return request.get("mode") == "release" and not _release_lease_id(request)


def _request_created_at(request: dict[str, Any], now: datetime) -> datetime | None:
    try:
        return datetime.fromisoformat(str(request["created_at"])).astimezone(now.tzinfo)
    except (KeyError, TypeError, ValueError):
        return None


def _archive_release_request(output_root: Path, path: Path, reason: str) -> Path | None:
    requests_root = output_root / "computer_interventions" / "requests"
    try:
        relative = path.relative_to(requests_root)
    except ValueError:
        return None
    destination = (
        output_root / "computer_interventions" / "archive" / "release" / reason / relative
    )
...
def archive_legacy_unleased_release_requests(
    output_root: Path,
    *,
    now: datetime | None = None,
) -> list[Path]:
    """Archive only releases that categorically cannot match any new lease.

    The grace window avoids racing a just-written request from an older client.
    We retain the original JSON in the private archive; it simply stops being
    eligible for agent dispatch.
    """
    moment = now or datetime.now().astimezone()
    archived: list[Path] = []
    for path in (output_root / "computer_interventions" / "requests").glob("*/*.json"):
        request = _safe_load(path)
        if not request or not _is_legacy_unleased_release(request):
            continue
        created_at = _request_created_at(request, moment)
        if created_at is None or moment - created_at < LEGACY_RELEASE_QUARANTINE_AFTER:
            continue
        destination = _archive_release_request(output_root, path, "legacy-unleased")
        if destination is not None:
            archived.append(destination)
    return archived


def _archive_completed_release_request(output_root: Path, request_id: str) -> Path | None:
    for path in (output_root / "computer_interventions" / "requests").glob("*/*.json"):
        request = _safe_load(path)
        if request and request.get("request_id") == request_id and request.get("mode") == "release":
            return _archive_release_request(output_root, path, "completed")
    return None


def _release_cancel_request_id(output_root: Path, request_id: str) -> str:
    for path in (output_root / "computer_interventions" / "requests").glob("*/*.json"):
        request = _safe_load(path)
        if not request or request.get("request_id") != request_id or request.get("mode") != "release":
            continue
        phone = request.get("phone", {})
        if isinstance(phone, dict) and phone.get("enabled"):
            return str(phone.get("cancel_request_id") or "").strip()
    return ""


def _request_files(output_root: Path, *, now: datetime | None = None) -> list[Path]:
    archive_legacy_unleased_release_requests(output_root, now=now)
    ...
    recent = paths[-80:]
    # Lease-bound release commands must never fall out of the scan window while
    # the computer is asleep or the agent is offline.  Unleased legacy records
    # are intentionally excluded: they cannot safely stop a later lease.
    durable = []
    for path in paths:
        try:
            ...
            continue
        if isinstance(request, dict) and request.get("mode") == "release" and _release_lease_id(request):
            durable.append(path)
...
def _reset_shared_episode_if_recovered(
    episode: dict[str, Any],
    offer: dict[str, Any],
    now: datetime,
) -> dict[str, Any]:
    """Apply the offer's recovery policy to the Pi-owned decline streak."""
    if int(episode.get("decline_streak", 0) or 0) <= 0:
        return episode
    policy = offer.get("decline_policy", {})
    observations = offer.get("observations", {})
    reset_reason = ""
    ...
    if meaningful >= float(policy.get("reset_when_meaningful_minutes_at_least", 20)):
        reset_reason = "meaningful_activity_recovered"
    elif rest >= float(policy.get("reset_when_confirmed_rest_minutes_at_least", 10)):
        reset_reason = "confirmed_rest_recovered"
    else:
        try:
            last_decline = datetime.fromisoformat(str(episode["last_decline_at"])).astimezone(now.tzinfo)
        except (KeyError, ValueError):
            last_decline = None
        if last_decline is not None:
            elapsed_minutes = (now - last_decline).total_seconds() / 60
            if elapsed_minutes >= float(policy.get("episode_reset_minutes", 90)):
                reset_reason = "episode_timeout"
    if reset_reason:
        episode["decline_streak"] = 0
        episode["last_reset_at"] = now.isoformat(timespec="seconds")
        episode["last_reset_reason"] = reset_reason
    return episode
...
def latest_pending_device_request(
    ...
    for path in reversed(_request_files(output_root, now=now)):
        try:
            request = load_json(path)
        except (OSError, ValueError):
            continue
        ...
        if not request_id or request_id in completed:
            continue
        if _is_legacy_unleased_release(request):
            continue
...
def resolve_intervention_decision(
    ...
    with _DECISION_LOCK:
        ...
        episode = _reset_shared_episode_if_recovered(
            _episode(output_root), offer, now
        )
        ...
        if decision == "accepted":
            ...
            if after >= 2:
                ...
                episode["last_decline_at"] = now.isoformat(timespec="seconds")
        if effective in {"accepted", "forced"}:
            ...
            execution = {
                **offer,
                ...
                # The phone may stay asleep after the decision.  Android applies
                # the 30m/20m/expired duration policy while polling for up to 15m.
                "expires_after_seconds": 960,
            }
        ...
        episode["last_updated_at"] = now.isoformat(timespec="seconds")
...
def receive_computer_intervention_event(
    ...
    *,
    ...
    """Record an event once; terminal request IDs are idempotent across retries."""
    with _EVENT_LOCK:
        return _receive_computer_intervention_event(
            output_root, event, user_agent=user_agent
        )


def _receive_computer_intervention_event(
    output_root: Path,
    event: dict[str, Any],
    *,
    user_agent: str = "",
    ...
    if request_id in set(state.get("completed_request_ids", [])):
        return {
            "ok": True,
    ...
    if decision in FINAL_DECISIONS or bool(event.get("final")):
        cancelled_request_id = _release_cancel_request_id(output_root, request_id)
        terminal_ids = [request_id] + ([cancelled_request_id] if cancelled_request_id else [])
        completed = list(dict.fromkeys(state.get("completed_request_ids", []) + terminal_ids))
        ...
        _archive_completed_release_request(output_root, request_id)
