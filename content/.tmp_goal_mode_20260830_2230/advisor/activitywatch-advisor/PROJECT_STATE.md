...
<!-- ai_provenance: source=codex; date=2026-08-28; verification=device-and-pi-checked; retrieved_notes="Pi activitywatch-advisor production state and phone usage receiver" -->
...
Pi 已上线按“任务 ID + 日期”记账的完成账本。普通任务与受支持的每周循环任务完成后仍保留在“今天”，灰显且不可编辑，同时从开放任务和 Agent 输入排除。循环任务复用既有 mutation queue 推进模板，无需修改 Obsidian 插件或 Agent。

每日计划番茄数单调保存；次日 04:10 后，若前一天计划至少 7 个且全部完成，日历显示大勾并发放一次直接高级植株机会。

## 2026-08-10 状态更新：手动专注手机锁机按剩余时间梯次下降

==Pi 权威 Focus Garden 与 Advisor 已改为向手机下发绝对 `focus_deadline_at`、可配置 `allowed_phone_minutes` 和 `nearest_remaining_midpoint_tie_longer` 策略。手机在首次实际尝试前按“剩余专注时间最接近哪个允许档位”选时长，精确中点取较长档；开始尝试后冻结该档位。默认档位仍为 `5,20,30,40,45,60`，配置入口为 `/home/conrad/services/focus-garden/config/settings.json` 的 `focus.allowed_minutes`。==

==暂停、取消和自然结束现在会分别给 phone 与 windows 创建独立、幂等的 release request。手机 release 会终止仍在等待或重试的原请求，Advisor 在确认 release 时同时把原 execute ID 记为完成，防止旧 pending 再出现。Pi 端 Advisor 13/13、Garden 20/20、Android 纯策略与离线 Gradle 构建通过；两项服务 active。Android `1.3.4 (19)` 已覆盖安装，Pi 心跳确认 1.3.4、公网轮询、无障碍与通知监听均正常。==

<!-- ai_provenance: source=codex; date=2026-08-10; verification=pi-tests-and-local-android-build; retrieved_notes="PI_SERVER_HANDOFF.md,我的专注花园/专注花园桥接手机APP.md" -->
## 2026-08-12：拖延任务标记与 Next Action 优先级

Focus Garden 的“推迟一天”现由独立 `postpone` 意图进入 Advisor；Pi 在 `data/task_sync/state.json` 中累计每个任务实际向后移动的天数。累计推迟 1 天只记录，达到 2 天即输出 `procrastinated=true`，并在当前待办和任务日历任务卡显示“拖延 · N天”。普通日期编辑不累计；任务完成或删除后清除计数。旧版本没有可靠历史，因此从本次部署后的推迟操作开始累计。

Next Action 提示词版本为 `next-action-v1.4-procrastination-priority`。存在仍可行动的拖延任务时，候选排序、硬规则、模型结果校验和 fallback 均要求先选择拖延任务并明确累计天数；午休、深夜睡眠等既有非任务硬规则仍优先。
...
Steam 客户端、Steam 商店和当前已识别的 Steam 游戏现由确定性规则锁定为 `entertainment`。半小时主流程只统计 report scope 内带 `steam_entertainment` 标签的时长；每次 08/38 分检查中，该值严格超过 5 分钟便以独立 `steam_activity` 原因令 `would_intervene=true`，把 Cold Turkey `steam游戏` 加入共享提醒，不再依赖其他行为触发理由。第二次连续拒绝转强制执行；强制 Steam 锁定前 Windows 弹出不可关闭的 60 秒存档倒计时。

Focus Garden 的 `focus.always_windows_blocks=["steam游戏"]` 保证每次专注都会按 session 时长锁 Steam，即使只选手机或未来新增 profile；停止仍沿用 lease ownership 的 `-stop` 路径。2026-08-14 的初版夜间 lease 已由下述 2026-08-15 硬锁与解锁门槛替代。
...
23:30 现在由 Windows Agent 显示 60 秒 Steam 收尾框，可立即关闭精确游戏路径 `C:\steam\steamapps\common\Magical Girl Celesphonia\Game.exe` 并获得一次普通植物奖励，或按 15 分钟档延时，最晚到次日 01:00。无操作或延时到点后先给足 60 秒存档时间，再关闭游戏并使用 Cold Turkey `-lock` 硬锁至 12:00；窗口按控件实际请求高度动态扩展，数字和“秒”均保留完整下边距。

任务清单仅在今天和明天的任务上显示“设为主要”，每个日期至多一个；按钮位于原四按钮上一行右侧。当天整项完成的任务按各自 `tomatoes_total` 一次性授予番茄，累计至少 5 个且当天主要任务已经完成，Steam 解锁门槛才通过；未完成任务的 `2/4` 等中途进度不计，指标封顶显示 `5/5`。否则 12:00 后 Windows Agent 继续滚动续上 5 分钟硬锁，Pi 离线时不解锁。主要任务同步进入 Next Action，但只作既有硬规则之后的软性参考。

<!-- ai_provenance: source=codex; date=2026-08-15; verification=windows-19-tests-advisor-37-tests-garden-targeted-tests-live-api-and-browser-ui -->

## 2026-08-15 状态更新：系统状态显示 Steam 解锁原因

Focus Garden“系统状态”的健康面板最后新增“Steam 解锁指标”宽卡。它同时显示总体状态、当天完成番茄的当前值/要求值、今日主要任务是否设置及是否完成，并把所有阻塞原因合并展示。任务同步暂不可用时显示数据不可用并说明系统会保守保持锁定。

`GET /api/system-status` 新增不含任务标题和正文的 `steam_unlock` 摘要，读取 Advisor gate 最多等待 3 秒。浏览器验收确认该卡位于健康卡片列表最后；2026-08-15 当前实况为 `0/6`、未设置主要任务、未达成。

<!-- ai_provenance: source=codex; date=2026-08-15; verification=pi-targeted-tests-live-api-and-browser-dom -->

## 2026-08-15：控制指标可手动同步

Focus Garden 系统状态首屏新增“同步状态”。`POST /api/control/sync` 更新当天 D 与当前 M/D/W/L/A/F/U/R，保存到 `data/control-live.json`；冻结周结论继续由 `data/control-review.json` 提供。界面显示最近同步时间，并区分“数据已同步 · 决策冻结”与待周评审预览。
