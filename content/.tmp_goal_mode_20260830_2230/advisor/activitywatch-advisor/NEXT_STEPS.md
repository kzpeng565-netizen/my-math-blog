...
- [ ] 次日 04:10 后观察首条自然 `daily_achievements`，并实际领取、种植一次直接高级奖励；单元测试已覆盖幂等，仍需真实跨日观察。
- [ ] 只有确需在 Pi 完成 `every 4 weeks on Sunday` 等复杂周期时，才扩展 recurrence 解析与周期锚点测试；当前会安全拒绝。

## 2026-08-10 后续事项：Focus Bridge 1.3.4 真机安装与自然验收

- [x] ==已覆盖安装 `D:\MyFocusGarden\focus-bridge-android\app\build\outputs\apk\debug\app-debug.apk`；设备版本与 Pi 心跳均确认 `1.3.4 (19)`。==
- [ ] ==用一次可接受的自然手动专注验证：锁屏等待跨过相邻档位中点后，实际启动档位与剩余时间一致；专注结束前未启动时，应收到 phone release 并不再补锁。不要为验收创建长时间不可逆锁机。==

<!-- ai_provenance: source=codex; date=2026-08-10; verification=build-ready-device-install-pending; retrieved_notes="PROJECT_STATE.md" -->


## 2026-08-14 后续观察：Steam 自然窗口

- [ ] 2026-08-14 18:08 已完成修正后的首次自然触发：Steam 10.28 分钟、`steam_activity` 候选、ntfy accepted、Windows ack。后续只需在自然相邻窗口核对第二次拒绝后的 60 秒存档倒计时；不补发历史请求。
- [ ] 在下一次自然 23:29—23:31 检查 60 秒收尾框、完整倒计时字体、主动关闭奖励和最晚 01:00 的延时选项；次日 12:00 检查当天整项完成任务按计划番茄数累计至少 5 个且主要任务完成的双门槛，未完成任务的 `2/4` 等中途进度不得计入。不要为验收主动制造或提前解除硬锁。

<!-- ai_provenance: source=codex; date=2026-08-15; verification=implementation-complete-natural-boundary-pending -->

## 2026-08-15：控制层数据准入

- [ ] 不在第 28 天自动宣布数据充足；分别检查 W/L 日报覆盖、M 分类匹配、D 日快照连续性和 A/F/R 样本量。
