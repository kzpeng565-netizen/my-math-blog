...
网页完成循环任务时，Pi 先以 `task_id@YYYY-MM-DD` 幂等记录本次实例，再复用现有 `update` mutation 推进 Obsidian 模板；Pi 不直接写 Markdown。当前只允许精确的 `every week on <weekday>`，其他表达式安全拒绝。完成实例留在今天，但从开放任务和 Agent 输入排除。

每日挑战采用当天计划番茄数的单调快照，仅在次日 04:10 后结算；`planned >= 7 && completed >= planned` 时按日期只发一次独立高级种植权益，不折算为 3 个普通奖励。

### D90. 手动手机锁机以绝对截止时间和可配置梯次为准【有效】

==手动专注不得再按请求创建时的原始分钟数无限等待。Pi 必须下发绝对 `focus_deadline_at` 与本次允许档位快照；Android 在首次实际尝试前选择与剩余时间距离最小的非负档位，精确中点取较长档，选到 0 即过期。档位一旦开始尝试即冻结，避免三次重试期间跳档。==

==phone 与 windows 的 release 必须使用不同 request ID，避免任一设备先回 final 就归档另一设备尚未处理的命令。手机 release 的终态同时 supersede 原 execute ID；结束、暂停与取消都必须走这条取消链路。==
...
Steam 客户端、Steam 商店和已识别 Steam 游戏必须由 tag rule 锁定为 `entertainment`。时长只累计当前半小时 report scope，阈值采用严格 `> 5` 分钟；每次 08/38 分定时检查中，只要本窗口越过阈值，就以独立原因 `steam_activity` 令 `would_intervene=true`，不再要求高刺激或工作—娱乐切换等第二个理由。第二次连续拒绝转 forced；相邻窗口保留拒绝计数，距上次拒绝达到 90 分钟、恢复至少 20 分钟有意义活动或 10 分钟确认休息时重置。forced Steam 执行前必须提供 60 秒本地存档倒计时。

### D93. Steam 夜间收尾使用 Cold Turkey 硬锁，专注与半小时锁仍使用可回收 lease【有效】

Windows Agent 每日 23:30 弹出 60 秒收尾框：用户可立即关闭配置中精确路径对应的游戏并获得一次普通植物奖励，或按 15 分钟档延时，最晚只能选到次日 01:00；倒计时无操作或延时到点后先保留 60 秒存档时间，再关闭该游戏并以 Cold Turkey `-start "steam游戏" -lock <minutes>` 硬锁到 12:00。夜间硬锁不能由 Agent 中途 `-stop`。半小时强制锁和 Focus Garden 专注锁仍按 lease 所有权建立与回收。

### D94. 中午后的 Steam 解锁以“达到 5 个实际完成番茄且当天主要任务已完成”为双门槛【有效】

任务清单只允许今天和明天各设一个主要任务；标记保存在 Pi task-sync 状态，不写入 Obsidian Markdown。当天整项完成的任务按各自 `tomatoes_total` 一次性授予番茄，累计达到 5 个，并且当天主要任务已经完成，`steam_unlock_gate.eligible` 才为 true；未完成任务的 `tomatoes_completed` 中途进度（如 `2/4`）不计入，指标封顶显示 `5/5`。否则 Windows Agent 每分钟续上 5 分钟 Cold Turkey 硬锁，Pi 暂时不可达也按未满足处理。Next Action 只把主要任务作为拖延、时段与健康等既有规则之后的软性排序依据。

<!-- ai_provenance: source=codex; date=2026-08-15; verification=implemented-ui-verified-and-targeted-tested -->
