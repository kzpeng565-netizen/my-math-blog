...
def _screen_state_at(
    events: list[dict[str, Any]], when, stale_seconds: int
) -> tuple[str, dict[str, Any] | None]:
    screen_event = _latest_before(events, "screen", when)
    if screen_event is None:
        return "unknown", None
    age_seconds = (when - screen_event["_timestamp"]).total_seconds()
    if age_seconds > stale_seconds:
        return "unknown", screen_event
    return str(screen_event.get("state", "unknown")), screen_event


def extract_tablet_facts(
    settings: dict[str, Any], period_start, period_end, device_filter: str = "tablet"
) -> dict[str, Any]:
    ...
    for left, right in zip(ordered, ordered[1:]):
        ...
        state, screen_event = _screen_state_at(relevant, left, stale_limit)
