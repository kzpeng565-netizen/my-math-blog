...
def calculate_work_entertainment_mixing(
    ...
    work_bridge_minimum_seconds = float(
        settings.get("state_rules", {}).get(
    ...
    qualified_entertainment_intervals = {
        (str(item["start"]), str(item["end"])) for item in deviations
    }
    def is_work_bridge(segment: dict[str, Any]) -> bool:
        return (
            segment.get("activity") == "work"
            and float(segment.get("duration_seconds", 0)) >= work_bridge_minimum_seconds
        )
    qualified_work_entertainment = [
        "work"
        if is_work_bridge(segment)
        else "entertainment"
        for segment in segments
        if is_work_bridge(segment)
        or (
            segment.get("activity") == "entertainment"
            and (str(segment.get("start")), str(segment.get("end")))
            in qualified_entertainment_intervals
        )
    ]
    qualified_transitions = sum(
        left != right
        for left, right in zip(
            qualified_work_entertainment,
            qualified_work_entertainment[1:],
        )
    )
    work_segments = [
    ...
    return {
        ...
        "rule": {
            ...
            "shopping_is_not_scored": True,
            "work_entertainment_minimum_work_bridge_seconds": work_bridge_minimum_seconds,
            "work_bridge_comparison": "at_least",
        ...
        "qualified_work_entertainment_transition_count": qualified_transitions,
