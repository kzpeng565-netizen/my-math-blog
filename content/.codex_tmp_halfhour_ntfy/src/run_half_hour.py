...
def build_shadow_ntfy_message(
    ...
    title = f"影子判断建议核验 {start:%H:%M}—{end:%H:%M}"
    lines = [
        "半小时影子判断认为：如果正式模式已启用，这个窗口会建议干预。",
        "",
    ...
    lines.append("这里只发送影子提醒，不执行干预、不修改任务。")
    return title, "\n".join(lines)


def send_shadow_candidate_via_ntfy(
    intervention: dict[str, Any],
    start: datetime,
    end: datetime,
    *,
    ...
    title, message = build_shadow_ntfy_message(intervention, start, end)
...
def run(arguments) -> dict[str, Any]:
    ...
    shadow_ntfy_delivery = send_shadow_candidate_via_ntfy(
        intervention,
        start,
        end,
    ...
    atomic_write_json(shadow_ntfy_receipt_path, shadow_ntfy_delivery)
    state = {
    ...
    return {
        ...
        "pushplus": delivery,
        "shadow_ntfy": shadow_ntfy_delivery,
