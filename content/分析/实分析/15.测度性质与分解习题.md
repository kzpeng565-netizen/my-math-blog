...
$$
Lebesgue $\sigma$-代数 $\mathcal{L}(\mathbb{R})$ 是 $\mathcal{B}(\mathbb{R})$ 关于 $m$ 的完备化。对于任意 Lebesgue 可测集 $E \in \mathcal{L}(\mathbb{R})$，可以表示为：
$$E=B \cup N$$
其中 $B \in \mathcal{B}(\mathbb{R})$，且 $N$ 是一个 Lebesgue 零测集
在构造 $\mu_A$ 的 Carathéodory 外测度 $\mu_A^*$ 下，由单调性：
$$0 \le \mu_A^*(N) \le \mu_A(N_0)=0 \implies \mu_A^*(N)=0$$
因此，$N$ 在完备化的意义下也是 $\mu_A$ 可测的，且 $\mu_A(N)=0$。
