...
(iii)
对测度 $\mu$ 应用 L-R-N 定理，将其分解为关于 Lebesgue 测度 $m$ 的绝对连续部分和奇异部分：
$$\mu = \rho + \lambda$$
其中 $\rho \ll m$，$\lambda \perp m$。由 Radon-Nikodym 定理，存在局部可积函数 $f$，使得对任意 Borel 集 $E$ 有 $\rho(E) = \int_E f dm$。

现在我们要证明 $f = F'$ a.e.。考察任意区间 $(a, x]$ 的测度：
$$F(x) - F(a) = \int_a^x f(t) dt + \lambda((a, x])$$
接下来，对等式两边关于 $x$ 求导（在 Lebesgue 测度下几乎处处进行）：
