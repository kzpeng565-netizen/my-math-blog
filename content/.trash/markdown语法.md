...
| invs    | ^{-1}            |
$$
\int\frac{xe^{ x }}{(x+1)^{2}}dx\ =\ \int \frac{(x+1-1)e^{ x }}{(x+1)^{2}}\ =\ \int\frac{dx}{x+1}e^{ x } - \int \frac{1}{(x+1)^{2}}e^{ x }dx\ = \ \frac{e^{ x }}{x+1}+c
$$
$$
\lim_{ b \to x_{0} } \int_{a}^{b} (x^{2}+1) \, dx 
$$
$$
\int_{a}^{b} \dot{x}\hat{x}\bar{x}\tilde{x}\vec{x}\underline{x}\mathbf{x}\mathbf{x}\mathbf{x}\mathbf{x}\mathbf{x}\mathbf{x} \, dx
$$
...
[
    ...
    {trigger: /pa([A-Za-z])([A-Za-z])/, replacement: "\\frac{ \\partial [[0]] }{ \\partial [[1]] } ", options: "mA"},
