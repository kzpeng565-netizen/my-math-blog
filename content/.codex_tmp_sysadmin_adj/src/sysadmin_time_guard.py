...
def is_maintenance_item(item: dict[str, Any], classification: dict[str, Any]) -> bool:
    if is_context_bridge_item(item, classification):
        return False
...
class SysadminTimeGuardEngine:
    ...
    def step(self, now: datetime) -> dict[str, Any]:
        ...
        last_maintenance_at = _parse_iso(state.get("last_maintenance_at"))
        clear_after = timedelta(
            minutes=float(self.policy["activity"]["cooldown_clear_minutes"])
        )
        if (
            current != GuardState.IDLE
            and not decision.maintenance_detected
            and last_maintenance_at is not None
            and now >= last_maintenance_at + clear_after
        ):
            ...
            self._save_transition(
                ...
                reason="cooldown_clear_window_complete",
                decision=decision,
            )
