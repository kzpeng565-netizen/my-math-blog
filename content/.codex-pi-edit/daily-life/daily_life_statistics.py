...
def _sleep_boundary(output_root: Path, day: date) -> dict[str, Any]:
    evening = datetime.combine(day, time(20, 0), tzinfo=TIMEZONE)
    morning = datetime.combine(day + timedelta(days=1), time(9, 0), tzinfo=TIMEZONE)
    ...
    if gap_kind == "open_end":
        notes.append("09:00前没有下一次手机亮屏记录，无法确定早上拿起手机时间。")
        return {
