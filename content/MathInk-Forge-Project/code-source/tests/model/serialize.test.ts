...
const sampleStyle: StrokeStyleSnapshotV1 = {
  version: 1,
  color: "#1971c2",
  size: 5,
  tool: "pen",
  opacity: 0.86,
  pressure: { ...DEFAULT_PRESSURE_CONFIG, curve: "hard", gamma: 1.4 },
  freehand: { ...DEFAULT_FREEHAND_CONFIG, smoothing: 0.72, startTaper: 8 },
};
...
describe("document payload", () => {
  it("encodes with a v2 prefix", () => {
    expect(encodeDocument(makeDoc([sampleStroke])).startsWith("v2:")).toBe(true);
  });
  ...
  it("round-trips a complete per-stroke style snapshot", () => {
    const styled: Stroke = { ...sampleStroke, style: sampleStyle };
    const restored = decodeDocument(encodeDocument(makeDoc([styled]))).regions[0].strokes[0];
    expect(restored.style).toEqual(sampleStyle);
  });

  it("keeps legacy v1 strokes readable without inventing a stored snapshot", () => {
    const payload = `v1:${deflateToBase64(
      JSON.stringify({ regions: [{ strokes: [{ ...sampleStroke, pts: [0, 0, 128] }] }] }),
    )}`;
    const restored = decodeDocument(payload).regions[0].strokes[0];
    expect(restored.color).toBe(sampleStroke.color);
...
describe("decode error handling", () => {
  ...
  it("rejects a validly compressed future document before migration", () => {
    const payload = `v3:${deflateToBase64(JSON.stringify({ version: 3, regions: [] }))}`;
    expect(() => decodeDocument(payload)).toThrow(/unsupported future document version 3/);
  });
