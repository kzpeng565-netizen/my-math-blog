...
## [1.0.0-advanced] - 2026-08-20

Local advanced fork based on upstream InkedMark 1.3.0 commit
`25515b65ce0ea9de47271f9b41c7c55cbc2605fa`.

### Added

- Five default complete-style pens in a touch-friendly Pen Box, with create,
  edit/rename, copy, delete, reorder, JSON import/export, validation, and
  conflict-safe ids.
- Linear, Soft, Medium, and Hard pressure curves with calibrated input/output
  ranges, gamma, explicit fallback pressure, and pure unit-tested mapping.
- Schema-v2 per-stroke style snapshots; editing or deleting a reusable preset
  cannot change previously committed ink.
- Input Lab raw PointerEvent capture/export, pressure and timing capability
  statistics, 12 synthetic regression fixtures, and Desktop Replay through the
  production input/pressure/freehand path.
- Five generated non-sensitive upstream-v1 `.ink.md` compatibility fixtures.

### Safety

- Payloads from future document schemas are rejected and protected from
  overwrite by the inherited raw-byte echo path.
- The independent plugin id is `mathink-forge`; upstream and this fork are
  explicitly mutually exclusive within one Vault.
