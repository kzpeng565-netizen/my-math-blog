...
This repository is a local advanced fork of InkedMark, pinned to upstream commit
`25515b65ce0ea9de47271f9b41c7c55cbc2605fa` (upstream 1.3.0). It adds a versioned
Pen Box, pressure curves, per-stroke style snapshots, Huawei Input Lab capture,
and Desktop Replay. The plugin id and install directory are `mathink-forge`.

> [!CAUTION]
> Do not enable upstream `inkedmark` and `mathink-forge` in the same Vault.
> Both intentionally handle the same `.ink.md` and inline `inkedmark` formats.

For the v1.0 test/release procedure, see [MATHINK_FORGE_V1_QA.md](MATHINK_FORGE_V1_QA.md) and
[RELEASE_V1.0.md](RELEASE_V1.0.md). The remainder of this file is the upstream
README retained for inherited features and architecture.

For continuation or handoff, start with [HANDOFF.md](HANDOFF.md).

---

<p align="center">
  <img src="docs/assets/logo.svg" width="96" height="96" alt="Inherited InkedMark logo" />
</p>

# Inherited InkedMark documentation
...
1. Install and enable MathInk Forge (see [Install](#install)).
...
MathInk Forge makes network requests **only** for recognition, and only in three
...
- **MathInk Forge local build:** copy `main.js`, `manifest.json`, and
  `styles.css` into `<vault>/.obsidian/plugins/mathink-forge/`, then enable
  **MathInk Forge** in Community plugins.
- The upstream InkedMark BRAT/community installation instructions do not apply
  to this separately identified local fork.
