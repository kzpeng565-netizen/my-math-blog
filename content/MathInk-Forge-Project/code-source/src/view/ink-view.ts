...
import { Notice, Platform, TextFileView, apiVersion, type WorkspaceLeaf } from "obsidian";
import {
...
import { InkPersistenceState } from "../model/persistence";
...
import {
  Toolbar,
  applyPresetToToolbarState,
  type ToolbarState,
  toolbarStateFromPreset,
} from "./toolbar";
import type InkedMarkPlugin from "../main";
import type { InputFixture } from "../input/diagnostics";
...
export class InkView extends TextFileView {
  ...
  /** Preserves exact loaded bytes until a real edit; also guards suspect loads. */
  private readonly persistence = new InkPersistenceState();
  ...
  constructor(
    ...
    const preset =
      plugin.settings.penPresets.find((item) => item.id === plugin.settings.activePresetId) ??
      plugin.settings.penPresets[0];
    this.toolState = toolbarStateFromPreset(preset);
  }
  ...
  getViewData(): string {
    ...
    return this.persistence.output(() => buildInkFile(this.bodyText, this.doc));
  }
  ...
  setViewData(data: string, _clear: boolean): void {
    ...
    this.persistence.load(data, emptyReadOfRealFile || unreadableBlock);
    if (this.persistence.isProtected) {
      new Notice(
        "MathInk Forge: couldn't read this note's ink data (incomplete sync?). " +
  ...
  async recognize(provider: RecognitionProvider, auto = false): Promise<void> {
    if (this.persistence.isProtected) {
      if (!auto) {
        new Notice("MathInk Forge: this note is protected until its ink data loads cleanly.");
      }
      return;
    }
    ...
    if (strokes.length === 0) {
      if (readTextSection(this.bodyText) !== null) {
        ...
        if (!auto) new Notice("MathInk Forge: page is empty — cleared the transcription.");
      } else if (!auto) {
        new Notice("MathInk Forge: nothing to recognize yet.");
      }
      return;
    }

    if (provider.requiresNetwork && this.doc.recognizedHash === hash) {
      if (!auto) new Notice("MathInk Forge: transcription is already up to date.");
      return;
    }
    ...
    const progress =
      provider.id === MANUAL_PROVIDER_ID
        ? null
        : new Notice("MathInk Forge: recognizing handwriting…", 0);
    try {
      const result = await provider.recognize({
        strokes,
        onProgress: (message) => progress?.setMessage(`MathInk Forge: ${message}`),
      });
      if (result.text.trim()) {
        ...
        this.markDirtyAndRequestSave();
        new Notice("MathInk Forge: transcription added to the text layer — review and edit it.");
      } else if (!auto) {
        new Notice("MathInk Forge: manual transcription — type it in the text-layer panel.");
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new Notice(`MathInk Forge recognition failed: ${message}`, 8000);
    } finally {
      progress?.hide();
    }
  }
  ...
  private buildDom(): void {
    ...
    this.toolbar = new Toolbar(
      ...
      this.toolState,
      {
        ...
        onPresetSelect: (preset) => {
          this.surface?.setTool(preset.style.tool);
          void this.plugin.setActivePreset(preset.id);
        },
        onManagePresets: () => this.plugin.openPenBoxManager(() => this.refreshPenBox()),
      ...
      { presets: this.plugin.settings.penPresets },
    );
    ...
    this.surface = new InkSurface(
      ...
      },
      {
        ...
        isLocked: () => {
          if (!this.persistence.isProtected) return false;
          new Notice("MathInk Forge: this note is protected until its ink data loads cleanly.");
          return true;
        },
  ...
  startInputLab(): boolean {
    if (!this.surface || this.surface.isInputRecording) return false;
    const platform = Platform.isWin
      ? "Windows"
      : Platform.isMacOS
        ? "macOS"
        : Platform.isLinux
          ...
          : Platform.isIosApp
            ...
            : Platform.isAndroidApp
              ? "Android/HarmonyOS"
              : "unknown platform";
    this.surface.startInputRecording({
      name: `input-lab-${new Date().toISOString().replace(/[:.]/g, "-")}`,
      device: `${platform}; dpr=${window.devicePixelRatio}; ${this.plugin.settings.inputLabDeviceModel || "device model not entered"}`,
      app: `MathInk Forge ${this.plugin.manifest.version}; Obsidian API ${apiVersion}`,
      presetId: this.toolState.activePresetId ?? "custom-style",
      notes: "World-space PointerEvent capture; predicted samples are preview-only.",
      environment: {
        deviceModel: this.plugin.settings.inputLabDeviceModel,
        stylus: this.plugin.settings.inputLabStylus,
        operatingSystem: this.plugin.settings.inputLabOperatingSystem,
        obsidianVersion: this.plugin.settings.inputLabObsidianVersion,
        webViewVersion: this.plugin.settings.inputLabWebViewVersion,
        userAgent: "Not captured automatically; use the configured WebView version.",
      },
    });
    return true;
  }

  stopInputLab(): InputFixture | null {
    return this.surface?.stopInputRecording() ?? null;
  }

  get isInputLabRecording(): boolean {
    return this.surface?.isInputRecording ?? false;
  }

  /** Refresh presets without duplicating view event registrations. */
  private refreshPenBox(): void {
    this.toolbar?.setPresets(this.plugin.settings.penPresets);
    const preset = this.plugin.settings.penPresets.find(
      (item) => item.id === this.plugin.settings.activePresetId,
    );
    if (!preset) return;
    applyPresetToToolbarState(this.toolState, preset);
    this.surface?.setTool(preset.style.tool);
  ...
  private updateStatus(): void {
    ...
    const guard = this.persistence.isProtected ? " · ⚠ protected (sync)" : "";
  ...
  private markDirtyAndRequestSave(): void {
