...
function renderInlineBlock(
  ...
  edit.addEventListener("click", (event) => {
    ...
    try {
      ...
      new Notice(`MathInk Forge: couldn't open the handwriting editor — ${message}`, 8000);
    }
  });
}
...
async function writeInlineBlock(
  ...
  if (!file) {
    new Notice("MathInk Forge: couldn't save — the note is no longer available.");
    return;
  }
  ...
  if (!written) {
    new Notice(
      "MathInk Forge: couldn't find the handwriting block in the note (was it edited meanwhile?). " +
