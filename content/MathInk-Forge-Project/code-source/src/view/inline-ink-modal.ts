...
export class InlineInkModal extends Modal {
  ...
  constructor(
    ...
    const preset =
      settings.penPresets.find((item) => item.id === settings.activePresetId) ??
      settings.penPresets[0];
  ...
  async recognize(provider: RecognitionProvider, auto = false): Promise<void> {
    ...
    if (strokes.length === 0) {
      if (!auto) new Notice("MathInk Forge: nothing to recognize yet.");
      return;
    }
    const progress =
    ...
    try {
      ...
      if (text) {
        ...
        new Notice("MathInk Forge: transcription added as the caption — review and edit it.");
      } else if (!auto) {
        new Notice("MathInk Forge: manual transcription — type the caption in the text panel.");
        if (!this.showCaption) this.toggleCaptionPanel();
      }
    } catch (error) {
