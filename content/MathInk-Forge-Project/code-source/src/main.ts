...
import { normalizePresetList } from "./ink/presets";
import { PenBoxModal } from "./ui/pen-box-modal";
import { InputReplayModal } from "./ui/input-replay-modal";
import { inputFixtureBaseName, summarizeInput, type InputFixture } from "./input/diagnostics";
...
export default class InkedMarkPlugin extends Plugin {
  ...
  override async onload(): Promise<void> {
      ...
      editorCallback: (editor: Editor) => {
        ...
        new InlineInkModal(this, doc, "", (result) => {
          if (editor.getRange(from, to) !== block) {
            new Notice("MathInk Forge: the note changed — the drawing was not saved.");
            return;
          }
    ...
    this.addCommand({
      id: "start-input-lab-recording",
      name: "Input Lab: start raw input recording",
      checkCallback: (checking) => {
        const view = this.app.workspace.getActiveViewOfType(InkView);
        if (view?.isInputLabRecording) return false;
        if (view && !checking && view.startInputLab()) {
          new Notice("MathInk Forge: Input Lab recording started.");
        }
        return !!view;
      },
    });
    ...
    this.addCommand({
      id: "stop-input-lab-recording",
      name: "Input Lab: stop and export recording",
      checkCallback: (checking) => {
        const view = this.recordingInputLabView();
        if (!view) return false;
        if (!checking) {
          const fixture = view.stopInputLab();
          if (!fixture) {
            new Notice("MathInk Forge: no Input Lab recording is active.");
          } else {
            void this.exportInputLabFixture(fixture);
          }
        }
        return true;
      },
    ...
    this.addCommand({
      id: "open-desktop-replay",
      name: "Input Lab: open Desktop Replay",
      callback: () => new InputReplayModal(this.app, this.settings.penPresets).open(),
    });
  ...
  /** Find the recording view even when the command palette changed active focus. */
  private recordingInputLabView(): InkView | null {
    for (const leaf of this.app.workspace.getLeavesOfType(VIEW_TYPE_INK)) {
      if (leaf.view instanceof InkView && leaf.view.isInputLabRecording) return leaf.view;
    }
    return null;
  }

  /**
   * Persist Input Lab output through Obsidian's Vault API. Huawei's Android
   * WebView acknowledges Blob `<a download>` clicks without materializing a
   * file, so a visible vault file is the only deterministic cross-platform
   * export path.
   */
  private async exportInputLabFixture(fixture: InputFixture): Promise<void> {
    try {
      const folder = normalizePath("MathInk Forge Input Lab");
      if (!this.app.vault.getAbstractFileByPath(folder)) await this.app.vault.createFolder(folder);

      const base = inputFixtureBaseName(fixture);
      let path = "";
      for (let index = 0; ; index++) {
        const suffix = index === 0 ? "" : `-${index}`;
        const candidate = normalizePath(`${folder}/${base}${suffix}.json`);
      ...
      await this.app.vault.create(path, JSON.stringify(fixture, null, 2));
      const stats = summarizeInput(fixture);
      new Notice(
        `Input Lab saved to ${path}: ${stats.strokeCount} stroke(s), ${stats.committedSampleCount} samples, max gap ${stats.maxMoveGapMs.toFixed(1)} ms, max handler ${stats.processingMaxMs.toFixed(1)} ms.`,
        10000,
      );
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      new Notice(`MathInk Forge: Input Lab export failed: ${message}`, 10000);
    }
  }
  ...
  async loadSettings(): Promise<void> {
    ...
    this.settings.penPresets = normalizePresetList(this.settings.penPresets);
    if (!this.settings.penPresets.some((preset) => preset.id === this.settings.activePresetId)) {
      this.settings.activePresetId = this.settings.penPresets[0].id;
    }
  }

  async setActivePreset(id: string): Promise<void> {
    if (!this.settings.penPresets.some((preset) => preset.id === id)) return;
    this.settings.activePresetId = id;
    await this.saveSettings();
  }

  openPenBoxManager(onChange: () => void): void {
    new PenBoxModal(this, onChange).open();
  }
  ...
  async maybeShowScribbleNotice(): Promise<void> {
    ...
    await this.saveSettings();
    new Notice(
      "MathInk Forge tip: if handwriting drops strokes on iPad, turn off " +
        "Settings → Apple Pencil → Scribble. iPadOS intercepts fast Pencil " +
        "strokes before MathInk Forge can see them.",
      0,
    );
  }
  ...
  async setDebugHud(enabled: boolean, notify = false): Promise<void> {
    ...
    if (notify) new Notice(`MathInk Forge input debug overlay ${enabled ? "on" : "off"}`);
  }
