...
import type { FreehandConfigV1, PressureConfigV1, StrokeStyleSnapshotV1 } from "./stroke-style";
      ...
      strokes: region.strokes.map((stroke) => ({
        ...
        ...(stroke.style
          ? {
              style: {
                ...stroke.style,
                pressure: { ...stroke.style.pressure },
                freehand: { ...stroke.style.freehand },
              },
            }
          : {}),
      })),
...
function normalizeStrokeStyle(raw: unknown): StrokeStyleSnapshotV1 | undefined {
  if (!isRecord(raw) || raw.version !== 1) return undefined;
  if (raw.tool !== "pen" && raw.tool !== "highlighter") return undefined;
  if (typeof raw.color !== "string" || !/^#[0-9a-f]{6}$/i.test(raw.color)) return undefined;
  if (!finiteIn(raw.size, 0.25, 64) || !finiteIn(raw.opacity, 0, 1)) return undefined;

  const pressureRaw = isRecord(raw.pressure) ? raw.pressure : null;
  const freehandRaw = isRecord(raw.freehand) ? raw.freehand : null;
  if (!pressureRaw || !freehandRaw) return undefined;
  if (typeof pressureRaw.enabled !== "boolean") return undefined;
  if (
    !finiteIn(pressureRaw.inputMin, 0, 1) ||
    !finiteIn(pressureRaw.inputMax, 0, 1) ||
    pressureRaw.inputMax <= pressureRaw.inputMin ||
    !finiteIn(pressureRaw.outputMin, 0, 1) ||
    !finiteIn(pressureRaw.outputMax, 0, 1) ||
    !finiteIn(pressureRaw.gamma, 0.1, 5) ||
    !finiteIn(pressureRaw.fallback, 0, 1)
  ) {
    return undefined;
  }
  const curve = pressureRaw.curve;
  if (!(curve === "linear" || curve === "soft" || curve === "medium" || curve === "hard")) {
    return undefined;
  }
  if (
    !finiteIn(freehandRaw.thinning, -1, 1) ||
    !finiteIn(freehandRaw.smoothing, 0, 1) ||
    !finiteIn(freehandRaw.streamline, 0, 1) ||
    !finiteIn(freehandRaw.startTaper, 0, 512) ||
    !finiteIn(freehandRaw.endTaper, 0, 512)
  ) {
    return undefined;
  }

  const pressure: PressureConfigV1 = {
    enabled: pressureRaw.enabled,
    inputMin: pressureRaw.inputMin,
    inputMax: pressureRaw.inputMax,
    outputMin: pressureRaw.outputMin,
    outputMax: pressureRaw.outputMax,
    curve,
    gamma: pressureRaw.gamma,
    fallback: pressureRaw.fallback,
  };
  const freehand: FreehandConfigV1 = {
    thinning: freehandRaw.thinning,
    smoothing: freehandRaw.smoothing,
    streamline: freehandRaw.streamline,
    startTaper: freehandRaw.startTaper,
    endTaper: freehandRaw.endTaper,
  };
  return {
    version: 1,
    color: raw.color,
...
function normalizeStroke(raw: unknown, index: number): Stroke | null {
  ...
  const style = normalizeStrokeStyle(raw.style);
  return {
 ...
 * {@link InkDocument}. Future versions are rejected before this function runs.
 */
...
export function decodeDocument(payload: string, fallbackWidth = 1024): InkDocument {
  ...
  if (!Number.isInteger(version) || version < 1) throw new SerializeError("invalid version number");
  if (version > SCHEMA_VERSION) {
    throw new SerializeError(`unsupported future document version ${version}`);
  }
