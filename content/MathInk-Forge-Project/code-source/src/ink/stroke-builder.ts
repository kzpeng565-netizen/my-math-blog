...
import type { PressureConfigV1 } from "../model/stroke-style";
import { mapConfiguredPressure } from "./pressure";
...
export interface StrokeBuilderOptions {
  ...
  /** Versioned mapping used by MathInk Forge strokes. Omit for legacy behavior. */
  pressureConfig?: PressureConfigV1;
}
...
export function mapPressure(raw: number, options: StrokeBuilderOptions): number {
  if (options.pressureConfig) return mapConfiguredPressure(raw, options.pressureConfig);
