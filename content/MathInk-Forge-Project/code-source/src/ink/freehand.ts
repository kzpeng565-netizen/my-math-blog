...
export function optionsForStrokeStyle(style: StrokeStyleSnapshotV1): FreehandOptions {
  return {
    size: style.size,
    thinning: style.freehand.thinning,
    smoothing: style.freehand.smoothing,
    streamline: style.freehand.streamline,
    simulatePressure: !style.pressure.enabled,
    startTaper: style.freehand.startTaper,
    endTaper: style.freehand.endTaper,
  };
}
...
export function strokeOutline(
  ...
  return getStroke(toInputPoints(pts), {
    ...
    start: { taper: options.startTaper },
    end: { taper: options.endTaper },
    last: complete,
  });
}
