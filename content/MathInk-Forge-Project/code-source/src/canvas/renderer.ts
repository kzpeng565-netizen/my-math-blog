...
import { type Bounds, type InkDocument, type Stroke, strokeBounds } from "../model/document";
import type { ViewportState } from "./viewport";
import { type StrokeStyleSnapshotV1, legacyStrokeStyle } from "../model/stroke-style";

export type StrokeStyle = StrokeStyleSnapshotV1;
...
function get2dContext(
  ...
  if (!ctx) throw new Error("MathInk Forge: 2D canvas context unavailable");
  return ctx;
}
...
export class Renderer {
  ...
  private optionsFor(style: StrokeStyle): FreehandOptions {
    return optionsForStrokeStyle(style);
  }
    ...
    for (const region of doc.regions) {
      for (const stroke of region.strokes) {
        ...
        this.fillStroke(
          this.dryCtx,
          stroke.pts,
          styleOf(stroke, usePressure, this.highlighterAlpha),
          true,
        );
      }
    }
...
export function styleOf(
  ...
  return (
    stroke.style ??
    legacyStrokeStyle(stroke.color, stroke.size, stroke.tool, usePressure, highlighterAlpha)
  );
}
