...
export class LlmProvider implements RecognitionProvider {
  ...
  async recognize(req: RecognitionRequest): Promise<RecognitionResult> {
    ...
    if (vendor.userEndpoint && !cfg.baseUrl.trim()) {
      throw new Error("no endpoint URL set — add one in MathInk Forge settings.");
    }
    if (vendor.requiresKey && !cfg.apiKey.trim()) {
      throw new Error(`no API key set for ${vendorLabel} — add one in MathInk Forge settings.`);
    }
    ...
    if (isTruncatedLlmResponse(cfg.vendor, json)) {
      throw new Error(
        `${vendorLabel} hit MathInk Forge's ${MAX_OUTPUT_TOKENS}-token output limit before ` +
